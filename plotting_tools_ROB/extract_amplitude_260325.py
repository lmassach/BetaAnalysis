...
def extract_CFD_time(t_vals, a_vals, a_peak, frac):
    ...
    for i in range(len(a_vals) - 1):
        if a_vals[i] < A_CFD and a_vals[i + 1] >= A_CFD:
            t_CFD = t_vals[i] + (A_CFD - a_vals[i]) * (t_vals[i + 1] - t_vals[i]) / (
                a_vals[i + 1] - a_vals[i]
            )
            return t_CFD
...
def main():
    ...
    for i in range(1):
        ...
        for j in range(num_curves):
            ...
            if make_plots:
                ...
                ax.plot(
                    x_linspace,
                    1000 * np.polyval(parabola_coeffs, x_linspace),
                    "r",
                    label=r"$A_{\rm{para}}$: "
                ...
                ax.plot(
                    x_linspace,
                    1000 * gaussian(x_linspace, *params),
                    "g",
                    label=r"$A_{\rm{Gaus}}$: "
                ...
                ax.plot(
                    x_linspace,
                    1000 * lorentz_func(x_linspace),
                    "blue",
                    label=r"$A_{\rm{Lorentz}}$: "
                ...
                ax.plot(
                    x_linspace,
                    1000 * voigt_func(x_linspace),
                    "orange",
                    label=r"$A_{\rm{Voigt}}$: "
                    + str(round(1000 * voigt_peak, 2))
                    + " mV",
                    linewidth=3,
                    zorder=2,
                )
                ax.plot(
                    x_linspace,
                    1000 * spline_func(x_linspace),
                    "purple",
                    label=r"$A_{\rm{spline}}$: "
                    + str(round(1000 * spline_peak, 2))
                    + " mV",
                    linewidth=3,
                    zorder=2,
                )
                ...
                ax.plot(
                    x_linspace,
                    1000 * skewG_func(x_linspace),
                    "cyan",
                    label=r"$A_{\rm{skewed}}$: "
                    + str(round(1000 * skewG_peak, 2))
                    + " mV",
                    linewidth=3,
                    zorder=2,
                )
                ...
                ax.scatter(
                    reshaped_time_data[j][peak_idx - 3 : peak_idx + 4],
                    1000 * reshaped_ampl_data[j][peak_idx - 3 : peak_idx + 4],
                    s=200,
                    c="white",
                    marker="o",
                    edgecolor=edges[i],
                    ...
                    + ",\n"
                    + r"$A_{\rm{Sa}}$: "
                    + str(round(1000 * pmax, 2))
                    + " mV",
                    zorder=5,
                )
                ax.axvline(
                    tmax_for_plot, color="k", alpha=0.8, linestyle="--", linewidth=3
                )
    ...
    if cfd_studies:
        ...
        for j in range(num_curves):
            idx_para = np.where(np.array(reshaped_ampl_data[j]) > 0.0002 * a_para[j])[
                0
            ][0]
            print(len(reshaped_ampl_data[j]))
            print(len(a_gaus[j]))
            idx_gaus = np.where(np.array(reshaped_ampl_data[j]) > 0.0002 * a_gaus[j])[
                0
            ][0]
            idx_lorentz = np.where(
                np.array(reshaped_ampl_data[j]) > 0.0002 * a_lorentz[j]
            )[0][0]
            idx_voigt = np.where(np.array(reshaped_ampl_data[j]) > 0.0002 * a_voigt[j])[
                0
            ][0]
            idx_spline = np.where(
                np.array(reshaped_ampl_data[j]) > 0.0002 * a_spline[j]
            )[0][0]
            idx_landau = np.where(
                np.array(reshaped_ampl_data[j]) > 0.0002 * a_landau[j]
            )[0][0]
            ...
            mean_time_lorentz = np.mean(
                time_array_event[[idx_lorentz - 1, idx_lorentz]]
            )
            ...
            if time_res_calc:
                idx_para_mcp = np.where(np.array(rad_mcp[j]) > 0.0002 * a_para_mcp[j])[
                    0
                ][0]
                idx_gaus_mcp = np.where(np.array(rad_mcp[j]) > 0.0002 * a_gaus_mcp[j])[
                    0
                ][0]
                idx_lorentz_mcp = np.where(
                    np.array(rad_mcp[j]) > 0.0002 * a_lorentz_mcp[j]
                )[0][0]
                idx_voigt_mcp = np.where(
                    np.array(rad_mcp[j]) > 0.0002 * a_voigt_mcp[j]
                )[0][0]
                idx_spline_mcp = np.where(
                    np.array(rad_mcp[j]) > 0.0002 * a_spline_mcp[j]
                )[0][0]
                idx_landau_mcp = np.where(
                    np.array(rad_mcp[j]) > 0.0002 * a_landau_mcp[j]
                )[0][0]
                time_array_event_mcp = np.array(rtd_mcp[j])
                mean_time_para_mcp = 0.5 * (
                    time_array_event_mcp[idx_para_mcp - 1]
                    + time_array_event_mcp[idx_para_mcp]
                )  # np.mean(time_array_event_mcp[[idx_para_mcp-1, idx_para_mcp]])
                mean_time_gaus_mcp = np.mean(
                    time_array_event_mcp[[idx_gaus_mcp - 1, idx_gaus_mcp]]
                )
                mean_time_lorentz_mcp = np.mean(
                    time_array_event_mcp[[idx_lorentz_mcp - 1, idx_lorentz_mcp]]
                )
                mean_time_voigt_mcp = np.mean(
                    time_array_event_mcp[[idx_voigt_mcp - 1, idx_voigt_mcp]]
                )
                mean_time_spline_mcp = np.mean(
                    time_array_event_mcp[[idx_spline_mcp - 1, idx_spline_mcp]]
                )
                mean_time_landau_mcp = np.mean(
                    time_array_event_mcp[[idx_landau_mcp - 1, idx_landau_mcp]]
                )
        ...
        if time_res_calc:
            ...
            ax2oneoff.hist(
                [cfd30_para, cfd30_para_mcp],
                bins=150,
                range=(-2, 1),
                alpha=0.6,
                color=["blue", "purple"],
                edgecolor="black",
                label=["cfd30_para", "cfd30_para_mcp"],
                histtype="stepfilled",
                linewidth=1.5,
            )
      ...
      """
        ...
        rms_diff_gaus = np.sqrt(
            np.mean((np.array(cfd30_data) - np.array(cfd30_gaus)) ** 2)
        ).round(3)
        axes[0, 1].hist(
            cfd30_data,
            bins=400,
        ...
        rms_diff_lorentz = np.sqrt(
            np.mean((np.array(cfd30_data) - np.array(cfd30_lorentz)) ** 2)
        ).round(3)
        axes[1, 0].hist(
            cfd30_data,
            bins=400,
        ...
        rms_diff_voigt = np.sqrt(
            np.mean((np.array(cfd30_data) - np.array(cfd30_voigt)) ** 2)
        ).round(3)
        axes[1, 1].hist(
            cfd30_data,
            bins=400,
        ...
        rms_diff_spline = np.sqrt(
            np.mean((np.array(cfd30_data) - np.array(cfd30_spline)) ** 2)
        ).round(3)
        axes[0, 2].hist(
            cfd30_data,
            bins=400,
        ...
        rms_diff_landau = np.sqrt(
            np.mean((np.array(cfd30_data) - np.array(cfd30_landau)) ** 2)
        ).round(3)
        axes[1, 2].hist(
            cfd30_data,
            bins=400,
        ...
        if time_res_calc:
            ...
            axes[0, 0].plot(
                ...
                "r",
                linewidth=2,
                label=r"$\sigma_{tr}^{para}$ = " + str(round(data_tr_val, 1)) + " ps",
            )
            axes[0, 1].plot(
                x_fit,
                gaus_tr_fit,
                "g",
                linewidth=2,
                label=r"$\sigma_{tr}^{Gaus}$ = " + str(round(data_tr_val, 1)) + " ps",
            )
            axes[1, 0].plot(
                x_fit,
                lorentz_tr_fit,
            ...
            axes[1, 1].plot(
                ...
                "orange",
                linewidth=2,
                label=r"$\sigma_{tr}^{Voigt}$ = " + str(round(data_tr_val, 1)) + " ps",
            )
            axes[0, 2].plot(
                x_fit,
                spline_tr_fit,
                "purple",
                linewidth=2,
                label=r"$\sigma_{tr}^{spline}$ = " + str(round(data_tr_val, 1)) + " ps",
            )
            axes[1, 2].plot(
                x_fit,
                landau_tr_fit,
                "cyan",
                linewidth=2,
                label=r"$\sigma_{tr}^{Landau}$ = " + str(round(data_tr_val, 1)) + " ps",
            )
