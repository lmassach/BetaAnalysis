...
# myGlobalCut = "event>-1"
myGlobalCut = (
    "event>-1 && pmax[0] > 10 && pmax[0] < 150 && tmax[0] > 11 && tmax[0] < 14"
)
...
def get_fit_results(
    arr_of_results_to_fit,
    arr_of_biases,
    decomp_sigma=False,
    simplified=False,
    add_threshold_frac=None,
):
    arr_of_mean = []
    arr_of_sigma = []
    arr_of_ampl = []
    ...
    if decomp_sigma:
        if len(arr_of_sigma) < 3:
            ...
            sig1 = np.sqrt(
                0.5
                * (arr_of_sigma[0] ** 2 + arr_of_sigma[2] ** 2 - arr_of_sigma[1] ** 2)
            )
            sig2 = np.sqrt(
                0.5
                * (arr_of_sigma[0] ** 2 + arr_of_sigma[1] ** 2 - arr_of_sigma[2] ** 2)
            )
            sig3 = np.sqrt(
                0.5
                * (arr_of_sigma[1] ** 2 + arr_of_sigma[2] ** 2 - arr_of_sigma[0] ** 2)
            )
...
def hist_tree_file_basics(
    tree, file, var, index, nBins, xLower, xUpper, biasVal, cut_cond, ch, toaThreshold
):
    if (
        var
        == "cfd["
        ...
        thisHist = root.TH1F(
            "hist" + biasVal, var + ";tn-tn+1 / ns ;Events", nBins, xLower, xUpper
        )
        tree.Draw(var + ">>hist" + biasVal, cut_cond)
    elif var == "cfd[" + str(ch) + "][2]-cfd[" + str(ch) + "][4]":
        thisHist = root.TH1F(
            "hist" + biasVal, var + ";t(20%)-t(50%) / ns ;Events", nBins, xLower, xUpper
        )
        tree.Draw(var + ">>hist" + biasVal, cut_cond)
    elif (
        var
        ...
        for i in range(nEntries):
            ...
            if eval(
                python_cut_cond,
                {},
                {"event": tree.event, "pmax": tree.pmax, "negpmax": tree.negpmax},
            ):
...
def tMaxTest(files, trees, ch, total_number_channels):
    ...
    if total_number_channels == 1:
        for i in range(len(trees)):
            ...
            print(files[i])
            print(bias)
            thisHist = hist_tree_file_basics(
                trees[i], files[i], var, i, nBins, xLower, xUpper, bias, cut_cond, ch, 0
            )
...
def landauToArea(files, trees, ch, total_number_channels):
    ...
    for i in range(len(arr_of_hists)):
        ...
        thisFit = plot_fit_curves(
            xLower, xUpper, "landau", arr_of_hists[i], i, arr_of_biases[i]
        )
        arr_of_fits.append(thisFit)
    ...
    print(frac_thres)
    landau_results = get_fit_results(
        arr_of_fits, arr_of_biases, add_threshold_frac=frac_thres
    )
...
def rmsNoise(files, trees, ch, total_number_channels):
    ...
    for i in range(len(arr_of_hists)):
        thisFit = plot_fit_curves(
            xLower, xUpper, "gaus", arr_of_hists[i], i, arr_of_biases[i]
        )
        arr_of_fits.append(thisFit)
        thisFit.Draw("SAME")
        # break
...
def timeRes(files, trees, ch, total_number_channels):
    ...
    if total_number_channels == 2:
        ...
        cut_cond = (
            "negpmax[0] > -15 && negpmax[0] < 5 && pmax[0] > 6.0 && pmax[0] < 120 && pmax[1] > 20 && pmax[1] < 120 && negpmax[1] > -15 && cfd[1]["
            + str(toaThreshold)
            + "]<2 && cfd[1]["
            + str(toaThreshold)
            + "]>-2 && cfd[0]["
        ...
        if ch == -1:
            ...
            for ch_it in range(
                3
            ):  # use this to force to look at first 3 channels, relevant for the 4-channel TRG-DUT-DUT-MCP test beam setup
                var = (
                    "cfd["
                    + str(ch_it)
                    + "]["
            ...
            cut_conds.append(
                ...
                + "] > 6.0 && pmax["
                + str(total_number_channels - 1)
                + "] < 120 && negpmax[0] > -15 && negpmax[0] < 5 && pmax[0] > 6.0 && pmax[0] < 120 && cfd[0]["
                + str(toaThreshold)
                + "]>-2.2 && cfd[1]["
                + str(toaThreshold)
                + "]>-2.5 && cfd[2]["
    ...
    if total_number_channels >= 3:
        range_of_vars = 3
    else:
        range_of_vars = len(vars)
    ...
    c1 = root.TCanvas(
        "c1",
        "Comparison of time resolution (CFD@"
        + str(toaThreshold + 1)
        + "0%) distribution",
        800,
        600,
...
def risingEdgeDiscretisation(files, trees, ch, total_number_channels):
    print(
        "Rising edge discretisation analysing temporal distribution between CFD @ 20% and 50%"
    )
    vars = []
    cut_conds = []
    toaThreshold = 4
    ...
    c1 = root.TCanvas(
        "c1", "Comparison of time resolution (CFD@20%-CFD@50%) distribution", 800, 600
    )
    c1.SetLogy()
    max_y = max(hist.GetMaximum() for hist in arr_of_hists) * 1.05
    arr_of_hists[0].GetYaxis().SetRangeUser(1, max_y)
    arr_of_hists[0].SetTitle(
        "Comparison of time resolution (CFD@20%-CFD@50%) distribution"
    )
    arr_of_hists[0].Draw("")
    for hist_to_draw in arr_of_hists[1:]:
        hist_to_draw.Draw("SAME")
...
def waveform(files, trees, ch, total_number_channels):
    ...
    ax.set_ylim(
        min(min(inner_array) for inner_array in w_data),
        max(max(inner_array) for inner_array in w_data),
    )
    ax.set_xlabel("t / 10 ns", fontsize=14)
    ax.set_ylabel("Amplitude / V", fontsize=14)
    ax.set_title(
        f"Waveform for events between Ev No.{event_start} and Ev No.{event_limit}",
        fontsize=16,
    )
...
def makeCSV(files, trees, total_number_channels):
    print(
        "CSV of information produced for plotting charge, RMS noise, and time resolution against bias"
    )
    ...
    for i, hist in enumerate(arr_of_hists):
        ...
        thisFit = plot_fit_curves(
            xLower, xUpper, "landau", hist_divided, i, arr_of_biases[i]
        )
        arr_of_fits.append(thisFit)
    ...
    for i in range(len(arr_of_hists)):
        ...
        arr_of_fits.append(thisFit)
    time_res = get_fit_results(
        arr_of_fits, arr_of_biases, decomp_sigma=True, simplified=True
    )

    merged_CC_RMS = pd.merge(
        charge_col.reset_index(),
        rms_noise.reset_index(),
        on=["index", "Bias"],
        how="inner",
    )
    merged_df = pd.merge(
        merged_CC_RMS, time_res.reset_index(), on=["index", "Bias"], how="inner"
    )
    merged_df.rename(
        columns={
            "Mean_x": "Charge",
    ...
    if os.path.exists(outcsvfile):
        ...
        if len(common_rows) != 0:
            print(
                "This bias point already exists in file '{outcsvfile}', check that there are no duplicate data points in the output file."
            )
        combined_df = pd.concat([existing_df, merged_df], ignore_index=True)
        combined_df.to_csv(outcsvfile, index=False)
        print(
            f"File '{outcsvfile}' exists, read and saved new data to it for use in bias plotter."
        )
    else:
...
def main():
    ...
    parser.add_argument(
        "--doTMAXTest", action="store_true", help="Enable the doTMAXTest option"
    )
    parser.add_argument(
        "--doPMAXTest", action="store_true", help="Enable the doPMAXTest option"
    )
    parser.add_argument(
        "--doNegPMAX", action="store_true", help="Enable the doNegPMAX option"
    )
    parser.add_argument(
        "--doLandauToArea", action="store_true", help="Enable the doLandauToArea option"
    )
    parser.add_argument(
        "--doChargeDist", action="store_true", help="Enable the doChargeDist option"
    )
    parser.add_argument(
        "--doRMSNoise", action="store_true", help="Enable the doRMSNoise option"
    )
    parser.add_argument(
        "--doTimeRes", action="store_true", help="Enable the doTimeRes option"
    )
    parser.add_argument(
        "--doDiscretisation",
        action="store_true",
        help="Enable the doDiscretisation of the CFD rising edge option",
    )
    parser.add_argument(
        "--doWaveform", action="store_true", help="Enable the doWaveform option"
    )
    parser.add_argument(
        "--csvOut",
        action="store_true",
        help="Output important data for plots against bias",
    )
    ...
    if args.ch == 0:
        ...
        total_number_channels = int(
            outer_vector_count / branch_with_number_channels.GetEntries()
        )
    ...
    if args.doTMAXTest:
        tMaxTest(file_array, tree_array, args.ch - 1, total_number_channels)
    if args.doPMAXTest:
        pMaxTest(file_array, tree_array, args.ch - 1, total_number_channels)
    if args.doNegPMAX:
        negPMax(file_array, tree_array, args.ch - 1, total_number_channels)
    if args.doLandauToArea:
        landauToArea(file_array, tree_array, args.ch - 1, total_number_channels)
    if args.doChargeDist:
        landauToCharge(file_array, tree_array, args.ch - 1, total_number_channels)
    if args.doRMSNoise:
        rmsNoise(file_array, tree_array, args.ch - 1, total_number_channels)
    if args.doTimeRes:
        timeRes(file_array, tree_array, args.ch - 1, total_number_channels)
    if args.doDiscretisation:
        risingEdgeDiscretisation(
            file_array, tree_array, args.ch - 1, total_number_channels
        )
    if args.doWaveform:
        waveform(file_array, tree_array, args.ch - 1, total_number_channels)
    if args.csvOut:
        makeCSV(file_array, tree_array, total_number_channels)
