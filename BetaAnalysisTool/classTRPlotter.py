    ...
    def run(self, file, file_index, tree, channel_array, mcp_tr):
        ...
        if (mcp_exists == True) & (mcp_tr != (0, 0)):
            for dut in duts_to_analyse:
                vars_cuts_defined = (
                    [x + y for x, y in zip(dut[0], mcp_channel[0])],
                    dut[1] + " && " + mcp_channel[1],
                    dut[2],
                )
        ...
        if (mcp_exists == False) | (mcp_tr == (0, 0)):
            for dut in duts_to_analyse:
                ...
                duts_vars_cuts.append(vars_cuts_defined)
            adapted_dut_channel = [
                second_plane[4:9] for second_plane in duts_to_analyse[1][0]
            ]
            dual_plane_var = [
                x + y for x, y in zip(duts_to_analyse[0][0], adapted_dut_channel)
            ]
            dual_plane_duts = [
                dual_plane_var,
                duts_to_analyse[0][1] + " && " + duts_to_analyse[1][1],
                int(mcp_channel[0][1][0]),
            ]
        ...
        for j in range(len(duts_vars_cuts)):
            ...
            for dut_var_ind, dut_var in enumerate(duts_vars_cuts[j][0]):
                thisHist = hist_tree_file_timeres(
                    tree,
                    file,
                    dut_var,
                    duts_vars_cuts[j][2],
        ...
        fit_results = get_fit_results_TR(
            arr_of_fits, arr_of_biases, arr_of_nevents, channel_of_dut, mcp_tr
        )
        return fit_results
