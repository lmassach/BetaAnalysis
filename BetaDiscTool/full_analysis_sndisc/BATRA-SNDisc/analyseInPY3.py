...
def main():
    parser = argparse.ArgumentParser(
        description="Read a text card containing information and location of ROOT analysis files and plot distributions of corresponding variables."
    )
    parser.add_argument(
        "config",
        type=str,
        help="Path to the configuration text card (e.g., config.txt)",
    )
    ...
    board_mapping = {4.7: "on SC board", 5: "on Mignone board", 1: "unmounted", 0: ""}
    ...
    if len(file_array) == 0:
        ...
        print(
            f"[BETA ANALYSIS] : [FILE READER] Total {len(file_array)} input ROOT files read."
        )
    ...
    for i, ch in enumerate(config["channels"]):
        print(
            f"        CH {i} : {channel_mapping.get(ch[0])} {board_mapping.get(ch[1])}"
        )

    data_out = []
    if config["pass_criteria"][0] == True:
        print(
            f"[BETA ANALYSIS]: [PLOTTER] Plotting PMAX distribution (note that on the first pass no selections are applied to the phase space)"
        )
        for file_ind, file_real in enumerate(file_array):
            plot_pmax = plotVar(
                "pmax",
                config["pass_criteria"][1],
                config["pass_criteria"][1],
                True,
    ...
    if config["pass_criteria"][0] == False:
        print(
            f"[BETA ANALYSIS] : [SIGNAL-NOISE DISCRIMINATOR] Running SNDisc NN to extract signal events from the ROOT file"
        )
        signal_event_array = []
        amplitude_dfs = []
        ...
        for file_ind, file_real in enumerate(file_array):
            signal_events, df_data = SNDisc_extract_signal(
                file_real,
                file_ind,
                tree_array[file_ind],
                config["channels"],
                config["pass_criteria"][1],
                "NN_performance/" + output_name_array[file_ind],
            )
        ...
        print(
            f"[BETA ANALYSIS] : [SIGNAL-NOISE DISCRIMINATOR] Signal events extracted and risetime, charge, RMS, and time resolution computation underway"
        )
        risetime_dfs = []
        for file_ind, file_real in enumerate(file_array):
            df_data = plot_gaussian(
                "risetime",
                file_real,
                file_ind,
                tree_array[file_ind],
                config["channels"],
                int(0.5 * config["pass_criteria"][1]),
                signal_event_array[file_ind],
                output_name_array[file_ind] + "_risetime",
            )
        ...
        for file_ind, file_real in enumerate(file_array):
            df_data = plot_gaussian(
                ...
                tree_array[file_ind],
                config["channels"],
                int(0.3 * config["pass_criteria"][1]),
                signal_event_array[file_ind],
                output_name_array[file_ind] + "_rms",
            )
        ...
        for file_ind, file_real in enumerate(file_array):
            df_data = plot_TR(
                ...
                tree_array[file_ind],
                config["channels"],
                int(0.2 * config["pass_criteria"][1]),
                signal_event_array[file_ind],
                output_name_array[file_ind] + "_TR",
                mcp_specs,
            )
