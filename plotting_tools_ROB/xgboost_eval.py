...
def compare_distributions(eval_file, results_file):
    ...
    plt.hist(
        pmax_real, bins=50, alpha=0.5, label="Real pmax", density=True, color="blue"
    )
    ...
    plt.hist(
        ...
        bins=50,
        alpha=0.5,
        label=f"Reconstructed pmax (weighted)\nS_KS = {ks_stat}, p_KS = {ks_p_value}",
        weights=weights,
        density=True,
        color="red",
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare real and reconstructed pmax distributions"
    )

    # Arguments for input files
    parser.add_argument(
        "--eval_file",
        help="Original ROOT file to compare to (e.g., eval_file.root)",
        required=True,
    )
    parser.add_argument(
        "--results_file",
        help="Evaluation results CSV file (e.g., evaluation_results.csv)",
        required=True,
    )
