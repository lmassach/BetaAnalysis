...
def main():
    parser = argparse.ArgumentParser(
        description="[L2 pre-proc]: merge ROOT files across channel into a single ROOT file."
    )
    parser.add_argument(
        "-i",
        "--input",
    ...
    for file_name in input_files:
        ...
        if tree:
            ...
            print(
                f"[L2 pre-proc]: Error retrieving TTree 'wfm' from file: {args.input + file_name}"
            )
    ...
    if not os.path.exists(args.output):
        print(
            f"[L2 pre-proc]: Output directory '{args.output}' does not exist. Creating it."
        )
