...
signal_times = norm.rvs(
    size=num_events // 2, loc=50, scale=5
)  # Gaussian for time (signal)
...
labels = np.concatenate(
    [np.ones(num_events // 2), np.zeros(num_events // 2)]
)  # 1 = signal, 0 = noise
...
for num_epochs in arr_num_epochs:
    ...
    for prob_threshold in arr_prob_threshold:
        ...
        bin_heights, bin_edges = np.histogram(
            max_amplitudes.numpy(), bins=200, range=(0, 20), density=True
        )
        bin_centres = (bin_edges[:-1] + bin_edges[1:]) / 2
        popt_gaussian, _ = curve_fit(
            gaussian, bin_centres, bin_heights, p0=[0.3, 2, 0.5]
        )
        ...
        popt_langaus, _ = curve_fit(
            langaus, bin_centres, filtered_heights, p0=[0.5, 5, 1.0, 1.0]
        )
        ...
        if np.isclose(prob_threshold / 0.05, np.round(prob_threshold / 0.05)) == True:
            ...
            plt.tight_layout()
            plt.savefig(
                "pmax_" + str(num_epochs) + "_" + str(prob_threshold)[2:] + ".png",
                facecolor="w",
            )
