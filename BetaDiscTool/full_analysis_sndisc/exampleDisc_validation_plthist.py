...
def binned_fit_langauss(samples, bins, min_x_val, max_x_val, channel, nan="remove"):
    ...
    hist, bin_edges = np.histogram(
        samples, bins, range=(min_x_val, max_x_val), density=True
    )
...
def langaus_sample(size, landau_loc=5, landau_scale=1.2, gauss_scale=0.5):
    landau_samples = (
        np.random.standard_cauchy(size) * landau_scale + landau_loc
    )  # Approximate Landau
...
file_list = [
    "../../../../../DESY_TB_Dec_24/data_TB/Run4_W5_RT_Ex1/stats_Sr_Run4_Ch1-125V_Ch2-125V_Ch3-2750V_trig230V.root"
]
file_array = []
tree_array = []
...
labels = np.concatenate(
    [np.ones(sig_events), np.zeros(num_events - sig_events)]
)  # 1 = signal, 0 = noise
...
class SignalProbabilityModel(nn.Module):
    def __init__(self):
        ...
        self.time_mu = nn.Parameter(
            torch.tensor(-0.5)
        )  # Gaussian center (signal-like time)
...
for num_epochs in arr_num_epochs:
    ...
    for epoch in range(num_epochs):
        ...
        loss = -torch.mean(
            labels * torch.log(scores + 1e-6)
            + (1 - labels) * torch.log(1 - scores + 1e-6)
        )
        loss.backward()
        optimizer.step()
        if epoch % 2000 == 0:
    ...
    for prob_threshold in arr_prob_threshold:
        ...
        popt, pcov, fitted_hist, bin_centres = binned_fit_langauss(
            data_var, nBins, xLower, xUpper, ch_ind
        )
        ...
        arr_qmax_frac.append(
            count_1p5max_bin / count_max_bin if count_max_bin > 0 else 0
        )
        ...
        bin_heights, bin_edges = np.histogram(
            max_amplitudes.numpy(), bins=150, range=(0, 150), density=True
        )
        ...
        popt_gaussian, _ = curve_fit(
            ...
            bin_centres,
            bin_heights,
            p0=[1 - (sig_events / num_events), 0.5 * ansatz_cut, 3.0],
        )
        A_gauss, mu_gauss, sigma_gauss = popt_gaussian
        filtered_heights, filtered_edges = np.histogram(
            filtered_amplitudes, bins=bin_edges, density=True
        )
    ...
    """
        ...
        if (
            np.isclose(
                prob_threshold / plot_every, np.round(prob_threshold / plot_every)
            )
            == True
        ):
            print(
                f"Number of epochs: {num_epochs} | Probability threshold: {prob_threshold}"
            )
            fig, axes = plt.subplots(1, 2, figsize=(12, 5))
            axes[0].hist(
                max_amplitudes.numpy(),
                bins=150,
                range=(0, 150),
                alpha=0.7,
                label="Signal + Noise",
            ...
            axes[0].plot(
                bin_centres,
                gaussian(bin_centres, *popt_gaussian),
                color="orange",
                linestyle="-",
                label=gaus_label,
                linewidth=2,
            ...
            axes[0].plot(
                bin_centres, langaus_scaled, "g-", label=langaus_label1, linewidth=2
            )
            ...
            axes[1].plot(
                filtered_centres,
                langauss.pdf(filtered_centres, *popt),
                "k--",
                label=langaus_label,
                linewidth=2,
            )
            ...
            axes[1].legend()
            axes[1].annotate(
                str(format(prob_threshold, "." + str(dec_p) + "f")),
                xy=(0.6, 0.5),
            ...
            plt.savefig(
                ...
                + str(num_epochs)
                + "_"
                + str(format(prob_threshold, "." + str(dec_p) + "f"))[2:]
                + ".png",
                facecolor="w",
            )
...
for i, y_column in enumerate(
    ...
    for j, label in enumerate(df["Number of epochs"].unique()):
        ...
        ax.plot(
            subset["Probability threshold"],
            subset[y_column],
            label=f"{label} ({y_column})",
            color=colours[j],
            marker=markers[j],
            markersize=8,
...
x_fit_quad = np.linspace(
    min(df["Signal event count"]), max(df["Signal event count"]), 100
)
...
for i, y_column in enumerate(["SSE score", "Red. Chi2 value", "Mod. Chi2 value"]):
    ...
    if i == 0:
        ax.plot(
            ...
            color="k",
            linestyle="--",
            label=f"Quadratic Fit: {coeffs_quad[0]:.2e}x² + {coeffs_quad[1]:.2e}x + {coeffs_quad[2]:.2e}",
        )
        ax.legend()
    # if i == 1:
