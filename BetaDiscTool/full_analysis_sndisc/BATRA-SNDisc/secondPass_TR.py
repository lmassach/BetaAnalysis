...
def plot_TR(
    ...
    if mcp_exists:
        ...
        for dut_val in SNDisc_signal_events_bool:
            filtered_mcp_cfd_for_this_dut = [
                val for val, keep in zip(data_filtered_signal_mcp, dut_val) if keep
            ]
            filtered_mcp_pmax_for_this_dut = [
                val for val, keep in zip(pmax_unselected_mcp, dut_val) if keep
            ]
        ...
        for dut_values in pmax_mcp_indexed_per_dut:
            indices = [
                i
                for i, val in enumerate(dut_values)
                if mcp_pmax_selection[0] <= val <= mcp_pmax_selection[1]
            ]
        ...
        final_filter_dut = filter_list_by_indices(
            data_filtered_signal_dut_set, selected_indices_per_dut
        )
        final_filter_mcp = filter_list_by_indices(
            cfd_mcp_indexed_per_dut, selected_indices_per_dut
        )
    ...
    for data_dut_ind, data_dut_val in enumerate(final_filter_dut):
        ...
        fig.add_trace(
            go.Histogram(
                x=data_var,
                xbins=dict(
                    start=max(-3, min(data_var)),
                    end=max(data_var),
                    size=(max(data_var) - min(data_var)) / nBins,
                ),
                name="Histogram",
        ...
        if not os.path.exists(var):
            os.makedirs(var)
        fig.write_image(
            var + "/" + savename + "_Ch" + str(ch_literal_index[data_dut_ind]) + ".png"
        )
        print(
            "[BETA ANALYSIS]: [GAUSSIAN PLOTTER] Saved file "
            + var
            + "/"
            + savename
            + "_Ch"
            + str(ch_literal_index[data_dut_ind])
            + ".png"
        )
