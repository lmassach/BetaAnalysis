...
def main():
    parser = argparse.ArgumentParser(
        description="[L1 pre-proc]: merge ROOT files across a channel into a single ROOT file for said channel."
    )
    parser.add_argument(
        "-i",
        "--input",
        required=True,
        help="Path to output directory (do not need to state the files, just the path)",
    )
    parser.add_argument(
        "-o",
        "--output",
        required=True,
        help="Path of output directory (will create it if it doesn't exist)",
    )
    parser.add_argument(
        "-ch",
        "--channel",
        type=int,
        required=True,
        help="Channel number for the event files for that channel to be merged from L0",
    )
    ...
    if not os.path.exists(args.output):
        print(
            f"[L1 pre-proc]: Output directory '{args.output}' does not exist. Creating it."
        )
    ...
    sorted_tree = TTree(
        "wfm", "Skimmed tree containing waveform information ordered by event number"
    )
    ...
    for entry in entries:
        ...
        if entry_counter % total_number_entries == 0:
            print(
                f"[L1 pre-proc]: {entry_counter} events processed of total {total_number_entries*len(input_files)}"
            )
