...
def main():
    ...
    modified_channels = [
        [
            ...
            (
                ch[2][0],
                1000 if ch[2][1] == 0 else ch[2][1],
                -100 if ch[2][2] >= 0 else ch[2][2],
                -50 if ch[2][3] == 0 else ch[2][3],
                50 if ch[2][4] == 0 else ch[2][4],
            ),
        ]
        for ch in config["channels"]
    ]  # set +ve negpmax lower bounds to -100 mV if they are +ve or 0 in the textCard (and tmax cuts on full range if specified as 0,0)
    ...
    print(
        f"[BETA ANALYSIS] : [CARD READER] Analyser "
        + sentence
        + " distributions for the following setup:"
    )
