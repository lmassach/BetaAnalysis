...
vars_to_analyze = ["pmax", "tmax", "area_new", "uarea_new", "risetime", "tot", "rms"]
...
for entry in range(n_entries):
    ...
    for ch in [1, 2]:
        ...
        for var in ["pmax", "tmax", "area_new", "uarea_new", "risetime", "tot", "rms"]:
...
for var in vars_to_analyze:
    for ch in [1, 2]:
        ...
        mask = ~np.isnan(values) & ~np.isinf(values) & (np.abs(values) < 1e10)
...
plt.barh(
    ...
    height=bar_width,
    color="dodgerblue",
    label="20 μm (Ch 1) 200 V",
)
plt.barh(
    y + bar_width / 2, auc2, height=bar_width, color="red", label="20 μm (Ch 2) 150 V"
)
