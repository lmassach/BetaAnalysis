...
def get_fit_results(
    arr_of_results_to_fit, arr_of_biases, decomp_sigma=False, simplified=False
):
    arr_of_mean = []
    arr_of_sigma = []
    arr_of_ampl = []
...
def hist_tree_file_basics(
    ...
    if (
        ...
        thisHist = root.TH1F(
            "hist" + biasVal, "width[0][1];Width / ns;Events", nBins, xLower, xUpper
        )
...
def waveform(files, trees, ch, total_number_channels):
    ...
    ax.set_title(
        f"Waveform comparison, first {event_limit} events per channel", fontsize=16
    )
    ...
    for entry in trees[0]:
        width_value = (
            entry.width[0][1]
            if len(entry.width) > 0 and len(entry.width[0]) > 1
            else np.nan
        )
        area_value = (
            entry.area_new[0] / 4.7 if len(entry.area_new) > 0 else np.nan
        )  # SC + 20 dB
        # area_value = entry.area_new[0] / 5 if len(entry.area_new) > 0 else np.nan # Mignone + 40 dB
        if (
            entry.pmax[0] > 10.0
            and entry.pmax[0] < 120.0
            and entry.pmax[1] < 120.0
            and entry.cfd[1][1] < 2
            and entry.cfd[1][1] > -2
            and entry.cfd[0][1] > 10
        ):
    ...
    for entry in trees[1]:
        ...
        if (
            entry.pmax[0] > 6.0
            and entry.pmax[0] < 200.0
            and entry.pmax[1] < 200.0
            and entry.cfd[1][1] < 2
            and entry.cfd[1][1] > -2
            and entry.cfd[0][1] > 10
        ):
