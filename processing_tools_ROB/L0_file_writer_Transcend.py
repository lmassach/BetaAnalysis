...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="[L0 pre-proc]: merge ROOT files across a channel into a single ROOT file for said channel."
    )
    parser.add_argument(
        "-i",
        "--input",
        required=True,
        help="Path to input directory (do not need to state the files, just the path)",
    )
    parser.add_argument(
        "-o",
        "--output",
    ...
    for file in os.listdir(args.input):
        if CVAL + "--Trace--" in file:
            ...
            if not os.path.exists(args.output):
                print(
                    f"[L0 pre-proc]: Output directory '{args.output}' does not exist. Creating it."
                )
            ...
            if int(event_number) < 15:
                xy_data = pd.DataFrame(
                    {
                        "event": event_number,
                        "t": np.linspace(-1.00720939e-08, 1.00279064e-08, 402),
                        "w": np.zeros(402),
                    }
                )
            else:
                xy_data = pd.DataFrame(
                    {"event": event_number, "t": lecroy_data.x, "w": lecroy_data.y}
                )
            ...
            if index % 10000 == 0:
                ...
                root_file = ROOT.TFile(
                    f"{args.output}/SPS_TestBeam-2024_{CVAL}_{index}.root", "RECREATE"
                )
