...
plt.hist(
    true_pmax_noise / true_cfddiff30_noise, bins=200, range=(0, 200), edgecolor="red"
)
plt.hist(
    true_pmax_sig / true_cfddiff30_sig,
    bins=200,
    range=(0, 200),
    edgecolor="blue",
    alpha=0.6,
...
for num_epochs in arr_num_epochs:
    ...
    for prob_threshold in arr_prob_threshold:
        ...
        popt_langaus, _ = curve_fit(
            langaus,
            bin_centres,
            filtered_heights,
            p0=[sig_events / num_events, 2.5 * ansatz_cut, 3.0, 3.0],
        )
        ...
        chi2 = np.sum(
            (filtered_heights - langaus(bin_centres, *popt_langaus)) ** 2
            / (langaus(bin_centres, *popt_langaus) + 1e-10)
        )
        ...
        if (
            ...
            axes[1].plot(
                filtered_centres,
                langaus(filtered_centres, *popt_langaus),
                "k--",
                label=langaus_label,
                linewidth=2,
            )
...
for i, y_column in enumerate(["SSE score", "Red. Chi2 value", "Mod. Chi2 value"]):
    ...
    if i == 1:
        ax.plot(
            ...
            color="k",
            linestyle="--",
            label=f"Quadratic Fit: x^{A_opt[0]:.4f}",
        )
        ax.legend()
