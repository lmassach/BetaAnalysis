...
def direct_to_table(
    name_and_df_couples, channel_configs, output_savename, thickness_info
):
    ...
    for index, (var, df) in enumerate(name_and_df_couples):
        if var == "amplitude":
            ...
            df_rt = df_rt.rename(
                columns={"Mean": "Rise time / ps", "Sigma": "Rise time Unc / ps"}
            )
            ...
            df_charge.loc[:, "Frac above 1p5 MPV"] = df_charge[
                "Frac above 1p5 MPV"
            ].round(3)
            df_charge.loc[:, "Frac above 1p5 Max Bin"] = df_charge[
                "Frac above 1p5 Max Bin"
            ].round(3)
            df_charge = df_charge.rename(
                columns={
                    "Charge MPV": "Charge / fC",
                    "Landau width": "Landau Cpt Charge",
                    "Gaussian sigma": "Gaussian Cpt Charge",
                    "Frac above 1p5 MPV": "Frac Charge >1.5xMPV",
                    "Frac above 1p5 Max Bin": "Frac Charge >1.5xQmax",
                }
            )
            df_charge["Gain"] = 100 * (df_charge["Charge / fC"] / thickness_col).round(
                2
            )
            ...
            df_rms = df_rms.rename(
                columns={"Mean": "RMS Noise / mV", "Sigma": "RMS Unc / mV"}
            )
            ...
            df_dvdt = df_dvdt.rename(
                columns={"dvdt_2080 MPV": "dV/dt[20%:80%] / mV/ps"}
            )
            dfs_to_concat.append(df_dvdt)
        elif var == "timeres":
            if first_df_found == False:
            ...
            df_tr = df_tr.rename(
                columns={
                    "Resolution @ 30%": "TR @ 30% / ps",
                    "Uncertainty @ 30%": "TR Unc @ 30% / ps",
                    "Resolution @ 50%": "TR @ 50% / ps",
                    "Uncertainty @ 50%": "TR Unc @ 50% / ps",
                    "NEvents_TR": "N.Ev. [DUT:MCP]",
                }
            )
    ...
    if (
        ("Rise time / ps" in dfs_comb.columns)
        and ("Amplitude / mV" in dfs_comb.columns)
        and ("RMS Noise / mV" in dfs_comb.columns)
    ):
        dfs_comb["Approx Jitter / ps"] = dfs_comb["RMS Noise / mV"] / (
            dfs_comb["Amplitude / mV"] / dfs_comb["Rise time / ps"]
        )
        ...
        dfs_comb.loc[:, "Approx Jitter Unc / ps"] = dfs_comb[
            "Approx Jitter Unc / ps"
        ].round(1)
        dfs_comb["Jitter / ps"] = dfs_comb["RMS Noise / mV"] / dfs_comb["dV/dt / mV/ps"]
        unc_cpt_rms = dfs_comb["RMS Unc / mV"] / dfs_comb["RMS Noise / mV"]
        unc_cpt_dvdt = 0  # idk the unc for a Langaus fit
        dfs_comb["Jitter Unc / ps"] = dfs_comb["Jitter / ps"] * np.sqrt(
            unc_cpt_rms**2 + unc_cpt_dvdt**2
        )
        dfs_comb.loc[:, "Jitter Unc / ps"] = (1000 * dfs_comb["Jitter Unc / ps"]).round(
            1
        )
        dfs_comb.loc[:, "Jitter / ps"] = (1000 * dfs_comb["Jitter / ps"]).round(1)
        dfs_comb = dfs_comb.drop(columns=["dV/dt / mV/ps"])
        dfs_comb["Jitter[20%:80%] / ps"] = (
            dfs_comb["RMS Noise / mV"] / dfs_comb["dV/dt[20%:80%] / mV/ps"]
        )
        dfs_comb["Jitter[20%:80%] Unc / ps"] = dfs_comb[
            "Jitter[20%:80%] / ps"
        ] * np.sqrt(unc_cpt_rms**2 + unc_cpt_dvdt**2)
        dfs_comb.loc[:, "Jitter[20%:80%] Unc / ps"] = (
            1000 * dfs_comb["Jitter[20%:80%] Unc / ps"]
        ).round(1)
        dfs_comb.loc[:, "Jitter[20%:80%] / ps"] = (
            1000 * dfs_comb["Jitter[20%:80%] / ps"]
        ).round(1)
        ...
        if "TR @ 30% / ps" in dfs_comb.columns:
            ...
            dfs_comb["Landau TR Cpt / ps"], dfs_comb["Landau TR Unc / ps"] = (
                landau_tr_quad_fit(
                    dfs_comb["Charge / fC"],
                    dfs_comb["TR @ 30% / ps"],
                    dfs_comb["TR Unc @ 30% / ps"],
                )
            )
            ...
            dfs_comb.loc[:, "Landau TR Cpt / ps"] = dfs_comb[
                "Landau TR Cpt / ps"
            ].round(1)
            dfs_comb.loc[:, "Landau TR Unc / ps"] = dfs_comb[
                "Landau TR Unc / ps"
            ].round(1)
            dfs_comb["WF6 Param / ps/um"] = (
                dfs_comb["Landau TR Cpt / ps"] / dfs_comb["Thickness / um"]
            )
            dfs_comb["WF6 Param Unc / ps/um"] = (
                dfs_comb["WF6 Param / ps/um"]
                * dfs_comb["Landau TR Unc / ps"]
                / dfs_comb["Landau TR Cpt / ps"]
            )
            dfs_comb.loc[:, "WF6 Param / ps/um"] = dfs_comb["WF6 Param / ps/um"].round(
                2
            )
            dfs_comb.loc[:, "WF6 Param Unc / ps/um"] = dfs_comb[
                "WF6 Param Unc / ps/um"
            ].round(2)
