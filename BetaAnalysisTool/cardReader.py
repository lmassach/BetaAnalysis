...
def read_text_card(file_path):
    ...
    channel_type_mapping = {"DUT": 1, "MCP": 2, "REF": 3}
    channel_area_to_charge_mapping = {"SC": 4.7, "Mig": 5}
    channels = [[0, 1]] * 8
        ...
        for line in f:
            ...
            if match:
                ...
                if key == "files":  # Handle multi-line `files`
                    ...
                    thickness_str = (
                        parts[2]
                        if (len(parts) > 2) & (type_str.upper() != "MCP")
                        else "nDUT"
                    )

                    channel_type = channel_type_mapping.get(type_str.upper(), 0)
                    channel_value = channel_area_to_charge_mapping.get(
                        additional_str, 1
                    )
                    ...
                    match = re.match(
                        r"^\s*(\[\s*(?:-?\d+(?:\.\d+)?\s*,\s*)*-?\d+(?:\.\d+)?\s*\]|\[\s*\]|0)\s*,\s*(.*)$",
                        value,
                    )
                    if not match:
                        raise ValueError(
                            f"Invalid format for {key}: Must start with an array '[x,y,...]', '[]', or '0'."
                        )
                    ...
                    if raw_lower_bound == "[]" or raw_lower_bound == "0":
                        ...
                        lower_bound = list(
                            map(float, raw_lower_bound.strip("[]").split(","))
                        )
                    ...
                    if len(remaining_parts) != 4:
                        raise ValueError(
                            f"Invalid format for {key}: Must contain exactly 4 additional comma-separated values."
                        )

                    try:
                        upper_bound, additional_condition, tlow, thigh = map(
                            float, remaining_parts
                        )
                    except ValueError:
                        raise ValueError(
                            f"Invalid format for {key}: The last 4 values must all be floats."
                        )

                    channels[channel_index][2] = (
                        lower_bound,
                        upper_bound,
                    ...
                    if (
                        plot_flags.get(param_key, False) == True
                    ):  # Check if this plot is enabled
                        nBins, xLower, xUpper = map(float, value.split(","))
                        plot_params[param_key + "_params"] = (
                            int(nBins),
                            xLower,
                            xUpper,
                        )
