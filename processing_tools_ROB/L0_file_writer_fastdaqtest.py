...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="[L0 pre-proc]: merge ROOT files across channel into a single ROOT file."
    )
    parser.add_argument(
        "-i",
        "--input",
    ...
    for file in os.listdir(args.input):
        if CVAL + "--Trace--" in file:
            ...
            if (
                event_number == "00000"
                or event_number == "00001"
                or event_number == "00015"
            ):
                continue
            ...
            grouped_df = (
                df.groupby("event")
                .agg({"t": lambda x: x.tolist(), "w": lambda x: x.tolist()})
                .reset_index()
            )
            ...
            root_file = ROOT.TFile(
                f"{args.output}/SPS_TestBeam-2024_{CVAL}_{event_number}.root",
                "RECREATE",
            )
