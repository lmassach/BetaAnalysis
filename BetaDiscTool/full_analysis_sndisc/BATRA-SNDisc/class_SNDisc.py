...
class SignalProbabilityModel(nn.Module):
    ...
    def forward(self, amplitudes, pow, invjitter, area):
        ...
        invjitter_prob = torch.sigmoid(
            (invjitter - self.invjitter_mu) / self.invjitter_sigma
        )
        area_prob = torch.sigmoid((area - self.area_mu) / self.area_sigma)
        signal_score = amp_prob * pow_prob  # * area_prob

        noise_amp_pdf = torch.exp(
            -0.5 * ((amplitudes - self.noise_amp_mu) / self.noise_amp_sigma) ** 2
        )
        noise_amp_pdf = noise_amp_pdf / (self.noise_amp_sigma * np.sqrt(2 * np.pi))
        noise_pow_pdf = torch.exp(
            -0.5 * ((pow - self.noise_pow_mu) / self.noise_pow_sigma) ** 2
        )
        noise_pow_pdf = noise_pow_pdf / (self.noise_pow_sigma * np.sqrt(2 * np.pi))
        noise_invjitter_pdf = torch.exp(
            -0.5
            * ((invjitter - self.noise_invjitter_mu) / self.noise_invjitter_sigma) ** 2
        )
        noise_invjitter_pdf = noise_invjitter_pdf / (
            self.noise_invjitter_sigma * np.sqrt(2 * np.pi)
        )
        noise_area_pdf = torch.exp(
            -0.5 * ((area - self.noise_area_mu) / self.noise_area_sigma) ** 2
        )
...
def differential_programming_SigProbModel(
    num_epochs, pmax, width, area, risetime, rms, ansatz_pmax_value, learning_rate
):
    ...
    max_area = torch.tensor(
        pmax * rms / safe_risetime, dtype=torch.float32
    )  # test var, not actual area
    ...
    for epoch in range(num_epochs):
        ...
        loss = -torch.mean(
            labels * torch.log(scores + 1e-12)
            + (1 - labels) * torch.log(1 - scores + 1e-12)
        )
        loss.backward()
        optimizer.step()
        if epoch % 2000 == 0:
    ...
    return (
        model(max_amplitudes, max_pow, max_invjitter, max_area).detach().numpy(),
        max_amplitudes,
    )
