...
def binned_fit_gaussian(samples, nBins, nan="remove"):
    ...
    if mu_ansatz < 0:
        hist, bin_edges = np.histogram(
            ...
            bins=nBins,
            range=(
                min(0, mu_ansatz - 2 * sigma_ansatz),
                max(0, mu_ansatz + 2 * sigma_ansatz),
            ),
            density=True,
        )
    else:
        ...
        hist, bin_edges = np.histogram(
            ...
            range=(
                max(0, mu_ansatz - 2 * sigma_ansatz),
                min(2 * mu_ansatz, mu_ansatz + 2 * sigma_ansatz),
            ),
            density=True,
        )
    bin_centres = bin_edges[:-1] + np.diff(bin_edges) / 2
    ...
    popt, pcov = curve_fit(
        gaussian, bin_centres, hist, p0=[A_ansatz, mu_ansatz, sigma_ansatz]
    )
    A, mu, sigma = popt
...
def get_fit_results_TR(df_of_results, mcp_specs):
    ...
    arr_of_sig_total_30 = [
        np.sqrt(sig_fit**2 + (1 / 600)) for sig_fit in df_of_results["Sigma"]
    ]
    arr_of_unc_total_30 = (
        np.array(df_of_results["Uncertainty"])
        * np.array(df_of_results["Sigma"])
    ...
    if (mcp_specs is not None) & (mcp_specs != (0, 0)):
        ...
        for ch_ind, ch_val in enumerate(df_of_results["Sigma"]):
            ...
            sig30err = (
                np.sqrt(
                    (ch_val * df_of_results["Uncertainty"][ch_ind]) ** 2
                    + (mcp_tr * mcp_tr_err) ** 2
                )
                / sig30
            )
        ...
        sig_30_p1 = np.sqrt(
            0.5
            * (
                df_of_results["Sigma"][0] ** 2
                + df_of_results["Sigma"][2] ** 2
                - df_of_results["Sigma"][1] ** 2
            )
        )
        sig_30_p2 = np.sqrt(
            0.5
