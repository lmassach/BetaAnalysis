...
def fit_landau(x, y):
    ...
    try:
        ...
        print(
            "Landau fit failed—perhaps your data isn't 'Landau-y' enough? Try better initial guesses."
        )
...
def main():
    ...
    if make_populated_plots:
        ...
        fig.suptitle(
            f"Run5 W16 210V LGAD-PIN : Total {len(a_max)} signal events across three families of events",
            fontsize=25,
            fontweight="bold",
        )
    ...
    if time_res_calc:
        ...
        for fam_ind, fam_proper in enumerate(
            ...
            fig.suptitle(
                f"Run5 W16 210V LGAD-PIN : Total {len(fam_proper)} signal events of Family {fam_ind+1}",
                fontsize=25,
                fontweight="bold",
            )
            plt.tight_layout(rect=[0, 0, 1, 0.96])
    ...
    if make_timewalk_plots:
        ...
        deltaT_lorentz = np.where(
            deltaT_lorentz > 0.1, deltaT_lorentz - 0.1, deltaT_lorentz
        )
        deltaT_voigt = np.where(deltaT_voigt > 0.1, deltaT_voigt - 0.1, deltaT_voigt)
        deltaT_spline = np.where(
            deltaT_spline > 0.1, deltaT_spline - 0.1, deltaT_spline
        )
        deltaT_landau = np.where(
            deltaT_landau > 0.1, deltaT_landau - 0.1, deltaT_landau
        )
        ...
        if time_res_calc:
            ...
            bin_indices_lorentz = (
                np.digitize(deltaT_lorentz, bins_lorentz, right=False) - 1
            )
            bin_indices_voigt = np.digitize(deltaT_voigt, bins_voigt, right=False) - 1
            bin_indices_spline = (
                np.digitize(deltaT_spline, bins_spline, right=False) - 1
            )
            bin_indices_landau = (
                np.digitize(deltaT_landau, bins_landau, right=False) - 1
            )
            ...
            binned_lorentz = [
                np.where(bin_indices_lorentz == i)[0] for i in range(num_bins_timewalk)
            ]
            binned_voigt = [
                np.where(bin_indices_voigt == i)[0] for i in range(num_bins_timewalk)
            ]
            binned_spline = [
                np.where(bin_indices_spline == i)[0] for i in range(num_bins_timewalk)
            ]
            binned_landau = [
                np.where(bin_indices_landau == i)[0] for i in range(num_bins_timewalk)
            ]

            selected_tr_events_para = [
                cfd20_para[binned_para[i]] for i in range(num_bins_timewalk)
            ]
            selected_tr_events_gaus = [
                cfd20_gaus[binned_gaus[i]] for i in range(num_bins_timewalk)
            ]
            selected_tr_events_lorentz = [
                cfd20_lorentz[binned_lorentz[i]] for i in range(num_bins_timewalk)
            ]
            selected_tr_events_voigt = [
                cfd20_voigt[binned_voigt[i]] for i in range(num_bins_timewalk)
            ]
            selected_tr_events_spline = [
                cfd20_spline[binned_spline[i]] for i in range(num_bins_timewalk)
            ]
            selected_tr_events_landau = [
                cfd20_landau[binned_landau[i]] for i in range(num_bins_timewalk)
            ]
        ...
        fig.suptitle(
            f"Total {len(a_max)} signal events", fontsize=16, fontweight="bold"
        )
        plt.tight_layout(rect=[0, 0, 1, 0.96])
