...
ClassImp(Analyzer)
    ...
    : TObject(), pvoltage(voltage), ptime(time) {}

Analyzer::Analyzer() : TObject(), pvoltage(0), ptime(0) {}

Analyzer::Analyzer(const Analyzer &a) : TObject() { *this = a; }

Analyzer &Analyzer::operator=(const Analyzer &a) {
  pvoltage = a.pvoltage;
  ptime = a.ptime;
  return *this;
}
...
Analyzer::Find_Signal_Maximum(bool confineSearchRegion, double searchRange[2]) {
  ...
  if (confineSearchRegion) {
    for (std::size_t j = 0; j < npoints; j++) {
      if (this->ptime.at(j) >= searchRange[0] &&
          this->ptime.at(j) <= searchRange[1]) // zoom in to find the Pmax
      {
        if (firstPoint) {
          pmaxIndex = j;
          firstPoint = false;
...
Analyzer::Pmax_with_GausFit(const std::pair<double, unsigned int> Pmax,
  ...
  if (pmaxIndex > 5 && pmaxIndex < maxIndex - 5) {
    ...
    for (int i = 0; i < 7; i++) {
      ...
      if (Pmax.first * this->pvoltage.at(pmaxIndex - 3 + i) > 0)
        pmax_histo.Fill(this->ptime.at(pmaxIndex - 3 + i),
                        this->pvoltage.at(pmaxIndex - 3 + i));
      else {
...
std::pair<double, unsigned int>
Analyzer::Find_Negative_Signal_Maximum(bool confineSearchRegion,
...
std::pair<double, double> Analyzer::Negative_Pmax_with_GausFit(
    const std::pair<double, unsigned int> NegPmax, unsigned int maxIndex) {
...
double
Analyzer::Get_Negative_Tmax(const std::pair<double, unsigned int> NegPmax) {
...
double Analyzer::DC_Area(double baseline_correction) { //
  ...
  for (size_t j = 5; j < (this->pvoltage.size() - 10); j++) {
...
double Analyzer::Find_Pulse_Area(const std::pair<double, unsigned int> Pmax) {
  ...
  return pulse_area; // collected pulse area, assuming voltage is in V, time is
                     // in s
}

double
Analyzer::Find_Undershoot_Area(const std::pair<double, unsigned int> Pmax) {
...
double Analyzer::Pulse_Integration_with_Fixed_Window_Size(
  ...
  for (int j = imax; j > -1; j--) // find index of start of pulse
  {
    if (!found_20pmax) {
      ...
      {
        _20pmax_time = xlinearInter(this->ptime.at(j), this->pvoltage.at(j),
                                    this->ptime.at(j + 1),
                                    this->pvoltage.at(j + 1), _20pmax);
        found_20pmax = true;
      }
    }
    ...
    if (!found_10pmax) {
      ...
      {
        _10pmax_time = xlinearInter(this->ptime.at(j), this->pvoltage.at(j),
                                    this->ptime.at(j + 1),
                                    this->pvoltage.at(j + 1), _10pmax);
        found_10pmax = true;
      }
    }
  ...
  for (unsigned int i = 0; i < npoints; i++) {
    if (this->ptime.at(i) >= (start_time - tRange[0]) &&
        this->ptime.at(i) <= timeOfMaximum + tRange[1])
  ...
  if (integration_option.compare("Simpson") == 0) {
    for (std::size_t i = 0, max = integration_voltage_vector.size(); i < max;
         i++) {
      if (i == 0)
        pulse_area = pulse_area + (time_difference / 3.0) *
                                      (integration_voltage_vector.at(i));
      else if (i == integration_voltage_vector.size() - 1)
        pulse_area = pulse_area + (time_difference / 3.0) *
                                      (integration_voltage_vector.at(i));
      else if (i % 2 == 0)
        pulse_area = pulse_area + 2 * (time_difference / 3.0) *
                                      (integration_voltage_vector.at(i));
      else
        pulse_area = pulse_area + 4 * (time_difference / 3.0) *
                                      (integration_voltage_vector.at(i));
    }
  }
  //================================
  //===========Rectangluar=========
    ...
    for (std::size_t j = 0, max = integration_voltage_vector.size(); j < max;
         j++) {
      pulse_area =
          pulse_area + (time_difference)*integration_voltage_vector.at(j);
    }
  }
  //================================
...
double Analyzer::Pulse_Integration_with_Fixed_Window_Size_with_GausFit(
    const std::pair<double, double> Pmax, unsigned int imax,
    std::string integration_option, double t_beforeSignal,
    double t_afterSignal) {
...
double Analyzer::Pulse_Area_With_Linear_Interpolate_Edge(
    const std::pair<double, unsigned int> Pmax, std::string integration_option,
  ...
  for (unsigned int j = 0; j < iend; j++) {
    if (this->ptime.at(j) >= start_time) {
      ...
      pulse_area = pulse_area +
                   ((this->ptime.at(j) - start_time)) * this->pvoltage.at(j);
      break;
    }
  }
  ...
  if (integration_option.compare("Simpson") == 0) {
    ...
    for (std::size_t i = 0, max = integration_y.size(); i < max; i++) {
      if (i == 0)
        pulse_area =
            pulse_area + (time_difference / 3.0) * (integration_y.at(i));
      else if (i == integration_y.size() - 1)
        pulse_area =
            pulse_area + (time_difference / 3.0) * (integration_y.at(i));
      else if (i % 2 == 0)
        pulse_area =
            pulse_area + 2 * (time_difference / 3.0) * (integration_y.at(i));
      else
        pulse_area =
            pulse_area + 4 * (time_difference / 3.0) * (integration_y.at(i));
    }
  }
...
double Analyzer::Pulse_Area_With_Linear_Interpolate_Edge_with_GausFit(
    const std::pair<double, double> Pmax, unsigned int imax,
...
double Analyzer::Find_Rise_Time(const std::pair<double, unsigned int> Pmax,
...
double
Analyzer::Find_Rise_Time_with_GausFit(const std::pair<double, double> Pmax,
                                      unsigned int imax, double bottom,
                                      double top) {
...
double
Analyzer::Find_Fall_Time_with_GausFit(const std::pair<double, double> Pmax,
                                      unsigned int imax, double bottom,
                                      double top) {
...
double Analyzer::Find_Dvdt(const int fraction, const int ndif,
  ...
  if (ndif == 0) {
    dvdt = (this->pvoltage.at(ifraction + 1) - this->pvoltage.at(ifraction)) /
           time_difference;
  }

  else {
    dvdt = (this->pvoltage.at(ifraction + ndif) -
            this->pvoltage.at(ifraction - ndif)) /
...
double Analyzer::Find_Dvdt_with_GausFit(const int fraction, const int ndif,
                                        const std::pair<double, double> Pmax,
                                        unsigned int imax) {
...
double Analyzer::Find_Dvdt2080_with_GausFit(
    const int ndif, const std::pair<double, double> Pmax, unsigned int imax) {
  ...
  _20pmax_time =
      xlinearInter(this->ptime.at(ifraction), this->pvoltage.at(ifraction),
                   this->ptime.at(ifraction + 1),
                   this->pvoltage.at(ifraction + 1), pmax * 0.2);

  _80pmax_time =
      xlinearInter(this->ptime.at(ifraction2), this->pvoltage.at(ifraction2),
                   this->ptime.at(ifraction2 + 1),
...
double
Analyzer::Rising_Edge_CFD_Time(const double fraction,
                               const std::pair<double, unsigned int> Pmax) {
  ...
  if (failure)
    ...
    time_fraction =
        time_fraction +
        (this->ptime.at(ifraction + 1) - this->ptime.at(ifraction)) *
            (pmax * fraction / 100.0 - this->pvoltage.at(ifraction)) /
            (this->pvoltage.at(ifraction + 1) - this->pvoltage.at(ifraction));

  return time_fraction;
}

double Analyzer::Rising_Edge_CFD_Time_with_GausFit(
    const double fraction, const std::pair<double, double> Pmax,
    unsigned int imax) {
  ...
  // cout<< this->ptime.at(ifraction) << " " << this->pvoltage.at(ifraction) <<
  // endl; cout<< this->ptime.at(ifraction+1) << " " <<
  // this->pvoltage.at(ifraction+1) << endl; cout<< "time at 20: " <<
  // time_fraction << " voltage at 20: " << pmax*fraction/100.0 << endl; cout<<"
  // "<<endl;
  return time_fraction;
}

double Analyzer::Falling_Edge_CFD_Time_with_GausFit(
    const double fraction, const std::pair<double, double> Pmax,
    unsigned int imax) {
...
double Analyzer::Find_Time_At_Threshold_with_GausFit(
  ...
  if (pmax < thr) {
    ...
    timeAtThreshold = xlinearInter(
        timeBelowThreshold, this->pvoltage.at(timeBelowThreshold_index),
        this->ptime.at(timeBelowThreshold_index + 1),
...
double Analyzer::Find_Time_At_Threshold_Falling_Edge_with_GausFit(
    const double thresholdLevel, const std::pair<double, double> Pmax,
    unsigned int imax) {
  ...
  if (pmax < thr) {
    ...
    for (unsigned i = pmax_index; i < npoints; i++) {
      if (this->pvoltage.at(i) <= thr) {
        timeBelowThreshold_index = i;
        timeBelowThreshold = this->ptime.at(i);
        failure = false;
...
// Self explainatory
double
Analyzer::Find_Time_Over_Threshold(const double first_thresholdLevel,
                                   const std::pair<double, unsigned int> Pmax,
  ...
  if (pmax < thr1 || pmax < thr2) {
    ...
    timeAtThreshold1 = xlinearInter(
        timeBelowThreshold1, this->pvoltage.at(timeBelowThreshold1_index),
        this->ptime.at(timeBelowThreshold1_index + 1),
    ...
    timeAtThreshold2 = xlinearInter(
        timeBelowThreshold2, this->pvoltage.at(timeBelowThreshold2_index),
        this->ptime.at(timeBelowThreshold2_index + 1),
        this->pvoltage.at(timeBelowThreshold2_index + 1), thr2);
  }
  return TMath::Abs(timeAtThreshold1 - timeAtThreshold2);
}

// Similar to Find_Pulse_Area but start/end times of the pulse are defined
// analitically. Further checks are useful, there might be bugs. There inputs
// args not needed!
double Analyzer::New_Pulse_Area(const std::pair<double, double> Pmax,
                                unsigned int imax,
  ...
  if (Pmax.second > range[0] && Pmax.second < range[1]) {
    ...
    start_time =
        xlinearInter(_10pmax_time, _10pmax, _20pmax_time, _20pmax, 0.0);

    for (unsigned int j = imax; j < npoints - 1;
         j++) // find index of end of pulse
    {
      if (!found_20pmax_2) {
        if (this->pvoltage.at(j) <= _20pmax) // stop after crossing zero
        {
          _20pmax_time_2 = xlinearInter(this->ptime.at(j), this->pvoltage.at(j),
                                        this->ptime.at(j - 1),
                                        this->pvoltage.at(j - 1), _20pmax);
          found_20pmax_2 = true;
        }
      ...
      if (!found_10pmax_2) {
        if (this->pvoltage.at(j) <= _10pmax) // stop after crossing zero
        {
          _10pmax_time_2 = xlinearInter(this->ptime.at(j), this->pvoltage.at(j),
                                        this->ptime.at(j - 1),
                                        this->pvoltage.at(j - 1), _10pmax);
          found_10pmax_2 = true;
        }
    ...
    end_time =
        xlinearInter(_10pmax_time_2, _10pmax, _20pmax_time_2, _20pmax, 0.0);
    // cout<<end_time<<endl;
    ...
    // if(start_time > start_window && end_time < end_window &&
    // !isinf(start_time) && !isinf(end_time)){
    ...
    return pulse_area; // collected pulse area, assuming voltage is in volts,
                       // time is in seconds
...
// Similar to Find_Pulse_Area but start/end times of the pulse are defined
// analitically. Further checks are useful, there might be bugs. There are
// inputs args not needed!
double Analyzer::New_Undershoot_Area(const std::pair<double, double> Pmax,
                                     const std::pair<double, double> Pmin,
                                     unsigned int imin,
                                     std::string integration_option,
                                     double range[2]) {
  ...
  if (Pmax.second > range[0] && Pmax.second < range[1]) {
    ...
    for (int j = imin; j > 0; j--) // find index of start of pulse
    {
      if (!found_20pmin2) {
        ...
        {
          _20pmin_time2 = xlinearInter(this->ptime.at(j), this->pvoltage.at(j),
                                       this->ptime.at(j + 1),
                                       this->pvoltage.at(j + 1), _20pmin);
          found_20pmin2 = true;
        }
      }
      ...
      if (!found_10pmin2) {
        ...
        {
          _10pmin_time2 = xlinearInter(this->ptime.at(j), this->pvoltage.at(j),
                                       this->ptime.at(j + 1),
                                       this->pvoltage.at(j + 1), _10pmin);
          found_10pmin2 = true;
        }
      }
    ...
    start_time =
        xlinearInter(_10pmin_time2, _10pmin, _20pmin_time2, _20pmin, 0.0);

    for (unsigned int j = imin; j < npoints - 1;
         j++) // find index of end of pulse
    {
      if (!found_20pmin) {
        if (this->pvoltage.at(j) >= _20pmin) // stop after crossing zero
        {
          _20pmin_time = xlinearInter(this->ptime.at(j), this->pvoltage.at(j),
                                      this->ptime.at(j - 1),
                                      this->pvoltage.at(j - 1), _20pmin);
          found_20pmin = true;
        }
      ...
      if (!found_10pmin) {
        if (this->pvoltage.at(j) >= _10pmin) // stop after crossing zero
        {
          _10pmin_time = xlinearInter(this->ptime.at(j), this->pvoltage.at(j),
                                      this->ptime.at(j - 1),
                                      this->pvoltage.at(j - 1), _10pmin);
          found_10pmin = true;
        }
