...
def direct_to_table(
    ...
    for index, (var, df) in enumerate(name_and_df_couples):
        if var == "amplitude":
            ...
            df_charge.loc[:, "Frac above 1p5 Qmax"] = df_charge[
                "Frac above 1p5 Qmax"
            ].round(3)
            df_charge = df_charge.rename(
                columns={
                    ...
                    "Frac above 1p5 Qmax": "Frac Charge >1.5xQmax",
                }
            )
    ...
    if (
        ...
        dfs_comb["Jitter / ps"] = dfs_comb["RMS Noise / mV"] / (
        ...
        if "TR @ 30% / ps" in dfs_comb.columns:
            dfs_comb["Landau TR Cpt / ps"] = np.sqrt(
                dfs_comb["TR @ 30% / ps"] ** 2 - dfs_comb["Jitter / ps"] ** 2
            )
            ...
            dfs_comb["Landau TR Unc / ps"] = (
                np.sqrt(unc_cpt_jit**2 + unc_cpt_tr**2) / dfs_comb["Landau TR Cpt / ps"]
            )
