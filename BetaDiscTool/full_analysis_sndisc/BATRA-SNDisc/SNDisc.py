    ...
    for ch_ind, ch_val in enumerate(channel_array):
        ...
        if sensorType == 1:
            ...
            for entry in tree:
                ...
                width_list.append(
                    width_sig
                )  # to fit Gaus noise and Langaus signal on pmax/width@30%
        ...
        scores, max_amplitudes = differential_programming_SigProbModel(
            num_epochs,
            pmax,
            width,
            area,
            risetime,
            rms,
            ansatz_pmax[file_index],
            learning_rate,
        )
        df = pd.DataFrame(scores)
        df.to_csv(
            savename + "SNDisc_Ch" + str(ch_ind) + ".csv", index=False, header=False
        )
        ...
        print(
            f"Number of events in linear selection: {len(pmax[pmax > ansatz_pmax[file_index]])}"
        )
        ...
        for prob_threshold in arr_prob_threshold:
            ...
            if len(filtered_amplitudes) > 1.5 * len(
                pmax[pmax > ansatz_pmax[file_index]]
            ):
                continue
            ...
            if len(filtered_amplitudes) < 0.99 * len(
                pmax[pmax > ansatz_pmax[file_index]]
            ):
            ...
            popt, pcov, fitted_hist, bin_centres = binned_fit_langauss(
                filtered_amplitudes, nBins, 0, max_pmax, ch_ind
            )
            ...
            bin_heights, bin_edges = np.histogram(
                max_amplitudes.numpy(), bins=max_pmax, range=(0, max_pmax), density=True
            )
            ...
            popt_gaussian, _ = curve_fit(
                ...
                bin_heights,
                p0=[
                    1 - (np.count_nonzero(pmax >= ansatz_pmax[file_index]) / len(pmax)),
                    0.5 * ansatz_pmax[file_index],
                    0.5 * ansatz_pmax[file_index],
                ],
            )
            ...
            num_ev_above_1p5 = len(
                filtered_amplitudes[filtered_amplitudes > 1.5 * mu_lang]
            )
        ...
        print(
            f"Optimal number of filtered signal events: {len(np_ose[np_ose == True])}"
        )
        print(f"Optimal cut at {smallest_prob}")

        amplitude_df_one_channel = df.loc[df["Probability threshold"] == smallest_prob]
        amplitude_df_one_channel = amplitude_df_one_channel[
            [
                "Channel",
                "Bias",
                "Amplitude MPV",
