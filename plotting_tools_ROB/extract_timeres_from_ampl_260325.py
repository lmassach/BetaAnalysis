...
def main():
    ...
    if time_res_calc:
        ...
        for j in range(num_curves):
            ...
            if linfit_cfd:
                time_value_control = find_CFD_time_with_threshold(
                    time_array_event, ampl_array_event, ampl_array_event.max(), 0.3
                )
                time_value_gaus = find_CFD_time_with_threshold(
                    time_array_event, ampl_array_event, a_gaus[j], 0.3 / 1000
                )
                time_value_voigt = find_CFD_time_with_threshold(
                    time_array_event, ampl_array_event, a_voigt[j], 0.3 / 1000
                )
                time_value_spline = find_CFD_time_with_threshold(
                    time_array_event, ampl_array_event, a_spline[j], 0.3 / 1000
                )
        ...
        data_tr_params, data_tr_params_cov = gaussian_fit_binned_data(
            cfd20_data, "Data"
        )
        gaus_tr_params, gaus_tr_params_cov = gaussian_fit_binned_data(
            cfd20_gaus, "Gaussian"
        )
        voigt_tr_params, voigt_tr_params_cov = gaussian_fit_binned_data(
            cfd20_voigt, "Voigt"
        )
        spline_tr_params, spline_tr_params_cov = gaussian_fit_binned_data(
        ...
        data_tr_val, data_tr_val_err = propagate_uncertainty(
            data_tr_params, data_tr_params_cov, mcp_tr_est, mcp_tr_est_err
        )  # np.sqrt((1000*data_tr_params[2])**2 - mcp_tr_est**2)
        gaus_tr_val, gaus_tr_val_err = propagate_uncertainty(
            gaus_tr_params, gaus_tr_params_cov, mcp_tr_est, mcp_tr_est_err
        )  # np.sqrt((1000*gaus_tr_params[2])**2 - mcp_tr_est**2)
        voigt_tr_val, voigt_tr_val_err = propagate_uncertainty(
            voigt_tr_params, voigt_tr_params_cov, mcp_tr_est, mcp_tr_est_err
        )  # np.sqrt((1000*voigt_tr_params[2])**2 - mcp_tr_est**2)
        spline_tr_val, spline_tr_val_err = propagate_uncertainty(
            spline_tr_params, spline_tr_params_cov, mcp_tr_est, mcp_tr_est_err
        )  # np.sqrt((1000*spline_tr_params[2])**2 - mcp_tr_est**2)
        ...
        axes1.hist(
            ...
            color="gray",
            edgecolor="black",
            label=r"$\sigma_{t}(\rm{A_{\rm{Sa}}})$ = "
            + str(round(data_tr_val, 1))
            + r"$\pm$"
            + str(round(data_tr_val_err, 1))
            + " ps",
        )
        axes1.hist(
            cfd20_gaus,
            ...
            edgecolor="black",
            alpha=0.4,
            label=r"$\sigma_{t}(\rm{A_{\rm{Gaus}}})$ = "
            + str(round(gaus_tr_val, 1))
            + r"$\pm$"
            + str(round(gaus_tr_val_err, 1))
            + " ps",
        )  # + "\n" + r"$\Delta_{RMS}$ = " + str(rms_diff_gaus))
        axes1.plot(
            x_tr_fit, data_tr_fit, "k--", linewidth=2
        )  # , label=r"$\sigma_{\rm{DUT}}$ = " + str(round(data_tr_val, 1)) + r"$\pm$" + str(round(data_tr_val_err, 1)) + " ps")
        axes1.plot(
            x_tr_fit, gaus_tr_fit, "g", linewidth=2
        )  # , label=r"$\sigma_{\rm{DUT}}^{\rm{Gaus}}$ = " + str(round(gaus_tr_val, 1)) + r"$\pm$" + str(round(gaus_tr_val_err, 1)) + " ps")
        ...
        axes2.hist(
            ...
            edgecolor="black",
            alpha=0.4,
            label=r"$\sigma_{t}(\rm{A_{\rm{Voigt}}})$ = "
            + str(round(voigt_tr_val, 1))
            + r"$\pm$"
            + str(round(voigt_tr_val_err, 1))
            + " ps",
        ...
        axes2.plot(
            x_tr_fit, voigt_tr_fit, "orange", linewidth=2
        )  # , label=r"$\sigma_{\rm{DUT}}^{\rm{Voigt}}$ = " + str(round(voigt_tr_val, 1)) + r"$\pm$" + str(round(voigt_tr_val_err, 1)) + " ps")
        ...
        axes3.hist(
            ...
            edgecolor="black",
            alpha=0.4,
            label=r"$\sigma_{t}(\rm{A_{\rm{spline}}})$ = "
            + str(round(spline_tr_val, 1))
            + r"$\pm$"
            + str(round(spline_tr_val_err, 1))
            + " ps",
        ...
        axes3.plot(
            x_tr_fit, spline_tr_fit, "purple", linewidth=2
        )  # , label=r"$\sigma_{\rm{DUT}}^{\rm{spline}}$ = " + str(round(spline_tr_val, 1)) + r"$\pm$" + str(round(spline_tr_val_err, 1)) + " ps")
