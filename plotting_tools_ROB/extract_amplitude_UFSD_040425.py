...
def fit_landau(x, y):
    ...
    try:
        ...
        return (
            peak_y,
            peak_x,
            lambda x_new: landau_pdf(x_new, *popt),
            compute_errors(y, y_fit),
        )
    except RuntimeError:
...
def skewed_gaussian(x, A, mu, sigma, alpha):
    return (
        2 * A * np.exp(-((x - mu) ** 2) / (2 * sigma**2)) * norm.cdf(alpha * (x - mu))
    )
...
def fit_skewed_gaussian(x, y):
    ...
    try:
        ...
        return (
            peak_y,
            peak_x,
            lambda x_new: skewed_gaussian(x_new, *popt),
            compute_errors(y, y_fit),
        )
    except RuntimeError:
...
def main():
    ...
    for i in range(1):
        ...
        if add_noise:
            ...
            for j in range(num_curves):
                reshaped_ampl_data_with_noise.append(
                    reshaped_ampl_data[j] + 0.001 * noise
                )
        ...
        for j in range(num_curves):
            ...
            fam_disc = (
                reshaped_ampl_data[j][start_idx + 1]
                - reshaped_ampl_data[j][end_idx - 2]
            )
            if fam_disc < -0.002:
            ...
            t_Amax_para.append(
                x_fine[np.argmin(np.abs(res_effect_y - max(res_effect_y)))]
            )
            ...
            idx_below = np.where((y_peak < y_parabola_max) & (x_peak < x_parabola_max))[
                0
            ][-1]
            ...
            try:
                ...
                params_mcp, _ = opt.curve_fit(
                    gaussian, x_peak_mcp, y_peak_mcp, p0=p0_mcp
                )
            ...
            # Lorentzian
            try:
                lorentz_peak, lorentz_fit, lorentz_func, lorentz_errors = (
                    fit_lorentzian(x_peak, y_peak)
                )
                (
                    lorentz_peak_mcp,
                    lorentz_fit_mcp,
            ...
            # Voigt
            try:
                voigt_peak, voigt_fit, voigt_func, voigt_errors = fit_voigt(
                    x_peak, y_peak
                )
                voigt_peak_mcp, voigt_fit_mcp, voigt_func_mcp, voigt_errors_mcp = (
                    fit_voigt(x_peak_mcp, y_peak_mcp)
                )
                ...
                idx_below = np.where(
                    (y_peak < voigt_peak)
                    & (x_peak < x_fine[np.argmin(np.abs(y_fine - voigt_peak))])
                )[0][-1]
            ...
            # interp_spline
            try:
                spline_peak, spline_fit, spline_func, spline_errors = fit_cubic_spline(
                    x_peak, y_peak
                )
                spline_peak_mcp, spline_fit_mcp, spline_func_mcp, spline_errors_mcp = (
                    fit_cubic_spline(x_peak_mcp, y_peak_mcp)
                )
                ...
                idx_below = np.where(
                    (y_peak < spline_peak)
                    & (x_peak < x_fine[np.argmin(np.abs(y_fine - spline_peak))])
                )[0][-1]
            ...
            # landau
            landau_peak, landau_fit, landau_func, landau_errors = fit_landau(
                x_peak, y_peak
            )
            landau_peak_mcp, landau_fit_mcp, landau_func_mcp, landau_errors_mcp = (
                fit_landau(x_peak_mcp, y_peak_mcp)
            )
            ...
            idx_below = np.where(
                (y_peak < landau_peak)
                & (x_peak < x_fine[np.argmin(np.abs(y_fine - landau_peak))])
            )[0][-1]
            t_w_below_Amax_landau.append(x_peak[idx_below])

            # skewed Gaussian
            skewG_peak, skewG_fit, skewG_func, skewG_errors = fit_skewed_gaussian(
                x_peak, y_peak
            )
            skewG_peak_mcp, skewG_fit_mcp, skewG_func_mcp, skewG_errors_mcp = (
                fit_skewed_gaussian(x_peak_mcp, y_peak_mcp)
            )
            y_fine = landau_func(x_fine)
            ...
            idx_below = np.where(
                (y_peak < skewG_peak)
                & (x_peak < x_fine[np.argmin(np.abs(y_fine - skewG_peak))])
            )[0][-1]
    ...
    if make_populated_plots:
        ...
        gs = gridspec.GridSpec(
            7, 3, height_ratios=[1, 1, 1, 1, 1, 1, 1], width_ratios=[1, 1, 1]
        )
        ...
        ax1.scatter(
            ...
            s=20,
            edgecolors="black",
            label=r"Parabola / A$_{max}$"
            + "\nMAE = "
            + str(round(mae_fam1_para, 4))
            + " RMSE = "
            + str(round(rmse_fam1_para, 4))
            + "\nNumber of events = "
            + str(len(a_max_fam1)),
        )
        ...
        ax3.scatter(
            ...
            s=20,
            edgecolors="black",
            label=r"Voigt / A$_{max}$"
            + "\nMAE = "
            + str(round(mae_fam1_voigt, 4))
            + " RMSE = "
            + str(round(rmse_fam1_voigt, 4))
            + "\nNumber of events = "
            + str(len(a_max_fam1)),
        )
        ...
        ax4.scatter(
            ...
            s=20,
            edgecolors="black",
            label=r"Spline / A$_{max}$"
            + "\nMAE = "
            + str(round(mae_fam1_spline, 4))
            + " RMSE = "
            + str(round(rmse_fam1_spline, 4))
            + "\nNumber of events = "
            + str(len(a_max_fam1)),
        )
        ...
        ax5.scatter(
            ...
            label=r"Parabola / A$_{max}$"
            + "\nMAE = "
            + str(round(mae_fam2_para, 4))
            + " RMSE = "
            + str(round(rmse_fam2_para, 4))
            + "\nNumber of events = "
            + str(len(a_max_fam2)),
        )
        ...
        ax7.scatter(
            ...
            label=r"Voigt / A$_{max}$"
            + "\nMAE = "
            + str(round(mae_fam2_voigt, 4))
            + " RMSE = "
            + str(round(rmse_fam2_voigt, 4))
            + "\nNumber of events = "
            + str(len(a_max_fam2)),
        )
        ...
        ax8.scatter(
            ...
            label=r"Spline / A$_{max}$"
            + "\nMAE = "
            + str(round(mae_fam2_spline, 4))
            + " RMSE = "
            + str(round(rmse_fam2_spline, 4))
            + "\nNumber of events = "
            + str(len(a_max_fam2)),
        )
        ...
        ax9.scatter(
            ...
            label=r"Parabola / A$_{max}$"
            + "\nMAE = "
            + str(round(mae_fam3_para, 4))
            + " RMSE = "
            + str(round(rmse_fam3_para, 4))
            + "\nNumber of events = "
            + str(len(a_max_fam3)),
        )
        ...
        ax11.scatter(
            ...
            label=r"Voigt / A$_{max}$"
            + "\nMAE = "
            + str(round(mae_fam3_voigt, 4))
            + " RMSE = "
            + str(round(rmse_fam3_voigt, 4))
            + "\nNumber of events = "
            + str(len(a_max_fam3)),
        )
        ...
        ax12.scatter(
            ...
            label=r"Spline / A$_{max}$"
            + "\nMAE = "
            + str(round(mae_fam3_spline, 4))
            + " RMSE = "
            + str(round(rmse_fam3_spline, 4))
            + "\nNumber of events = "
            + str(len(a_max_fam3)),
        )
        ...
        ax13.scatter(
            ...
            s=20,
            edgecolors="black",
            label=r"Lorentz / A$_{max}$"
            + "\nMAE = "
            + str(round(mae_fam1_lorentz, 4))
            + " RMSE = "
            + str(round(rmse_fam1_lorentz, 4))
            + "\nNumber of events = "
            + str(len(a_max_fam1)),
        )
        ...
        ax14.scatter(
            ...
            label=r"Lorentz / A$_{max}$"
            + "\nMAE = "
            + str(round(mae_fam2_lorentz, 4))
            + " RMSE = "
            + str(round(rmse_fam2_lorentz, 4))
            + "\nNumber of events = "
            + str(len(a_max_fam2)),
        )
        ...
        ax15.scatter(
            ...
            label=r"Lorentz / A$_{max}$"
            + "\nMAE = "
            + str(round(mae_fam3_lorentz, 4))
            + " RMSE = "
            + str(round(rmse_fam3_lorentz, 4))
            + "\nNumber of events = "
            + str(len(a_max_fam3)),
        )
        ...
        ax16.scatter(
            ...
            s=20,
            edgecolors="black",
            label=r"Landau / A$_{max}$"
            + "\nMAE = "
            + str(round(mae_fam1_landau, 4))
            + " RMSE = "
            + str(round(rmse_fam1_landau, 4))
            + "\nNumber of events = "
            + str(len(a_max_fam1)),
        )
        ...
        ax17.scatter(
            ...
            label=r"Landau / A$_{max}$"
            + "\nMAE = "
            + str(round(mae_fam2_landau, 4))
            + " RMSE = "
            + str(round(rmse_fam2_landau, 4))
            + "\nNumber of events = "
            + str(len(a_max_fam2)),
        )
        ...
        ax18.scatter(
            ...
            label=r"Landau / A$_{max}$"
            + "\nMAE = "
            + str(round(mae_fam3_landau, 4))
            + " RMSE = "
            + str(round(rmse_fam3_landau, 4))
            + "\nNumber of events = "
            + str(len(a_max_fam3)),
        )
        ...
        ax19.scatter(
            ...
            s=20,
            edgecolors="black",
            label=r"Skewed Gaus / A$_{max}$"
            + "\nMAE = "
            + str(round(mae_fam1_skewG, 4))
            + " RMSE = "
            + str(round(rmse_fam1_skewG, 4))
            + "\nNumber of events = "
            + str(len(a_max_fam1)),
        )
        ...
        ax20.scatter(
            ...
            label=r"Skewed Gaus / A$_{max}$"
            + "\nMAE = "
            + str(round(mae_fam2_skewG, 4))
            + " RMSE = "
            + str(round(rmse_fam2_skewG, 4))
            + "\nNumber of events = "
            + str(len(a_max_fam2)),
        )
        ...
        ax21.scatter(
            ...
            label=r"Skewed Gaus / A$_{max}$"
            + "\nMAE = "
            + str(round(mae_fam3_skewG, 4))
            + " RMSE = "
            + str(round(rmse_fam3_skewG, 4))
            + "\nNumber of events = "
            + str(len(a_max_fam3)),
        )
        ...
        fig.suptitle(
            f"UFSD 3.2 W7 300V : Total {len(a_max)} signal events across three families of events",
            fontsize=25,
            fontweight="bold",
        )
    ...
    if time_res_calc:
        ...
        for j in range(num_curves):
            ...
            if linfit_cfd:
                time_value_control = find_CFD_time_with_threshold(
                    time_array_event, ampl_array_event, ampl_array_event.max(), 0.2
                )
                time_value_para = find_CFD_time_with_threshold(
                    time_array_event, ampl_array_event, a_para[j], 0.2 / 1000
                )
                time_value_gaus = find_CFD_time_with_threshold(
                    time_array_event, ampl_array_event, a_gaus[j], 0.2 / 1000
                )
                time_value_lorentz = find_CFD_time_with_threshold(
                    time_array_event, ampl_array_event, a_lorentz[j], 0.2 / 1000
                )
                time_value_voigt = find_CFD_time_with_threshold(
                    time_array_event, ampl_array_event, a_voigt[j], 0.2 / 1000
                )
                time_value_spline = find_CFD_time_with_threshold(
                    time_array_event, ampl_array_event, a_spline[j], 0.2 / 1000
                )
                time_value_landau = find_CFD_time_with_threshold(
                    time_array_event, ampl_array_event, a_landau[j], 0.2 / 1000
                )
                time_value_skewG = find_CFD_time_with_threshold(
                    time_array_event, ampl_array_event, a_skewG[j], 0.2 / 1000
                )

            else:
                time_value_control = find_CFD_time_with_threshold_spline(
                    time_array_event, ampl_array_event, ampl_array_event.max(), 0.2
                )
                time_value_para = find_CFD_time_with_threshold_spline(
                    time_array_event, ampl_array_event, a_para[j], 0.2 / 1000
                )
                time_value_gaus = find_CFD_time_with_threshold_spline(
                    time_array_event, ampl_array_event, a_gaus[j], 0.2 / 1000
                )
                time_value_lorentz = find_CFD_time_with_threshold_spline(
                    time_array_event, ampl_array_event, a_lorentz[j], 0.2 / 1000
                )
                time_value_voigt = find_CFD_time_with_threshold_spline(
                    time_array_event, ampl_array_event, a_voigt[j], 0.2 / 1000
                )
                time_value_spline = find_CFD_time_with_threshold_spline(
                    time_array_event, ampl_array_event, a_spline[j], 0.2 / 1000
                )
                time_value_landau = find_CFD_time_with_threshold_spline(
                    time_array_event, ampl_array_event, a_landau[j], 0.2 / 1000
                )
                time_value_skewG = find_CFD_time_with_threshold_spline(
                    time_array_event, ampl_array_event, a_skewG[j], 0.2 / 1000
                )
            ...
            if linfit_cfd:
                time_value_control_mcp = find_CFD_time_with_threshold(
                    time_array_event_mcp,
                    ampl_array_event_mcp,
                    ampl_array_event_mcp.max(),
                    0.2,
                )
                time_value_para_mcp = find_CFD_time_with_threshold(
                    time_array_event_mcp,
                    ampl_array_event_mcp,
                    a_para_mcp[j],
                    0.2 / 1000,
                )
                time_value_gaus_mcp = find_CFD_time_with_threshold(
                    time_array_event_mcp,
                    ampl_array_event_mcp,
                    a_gaus_mcp[j],
                    0.2 / 1000,
                )
                time_value_lorentz_mcp = find_CFD_time_with_threshold(
                    time_array_event_mcp,
                    ampl_array_event_mcp,
                    a_lorentz_mcp[j],
                    0.2 / 1000,
                )
                time_value_voigt_mcp = find_CFD_time_with_threshold(
                    time_array_event_mcp,
                    ampl_array_event_mcp,
                    a_voigt_mcp[j],
                    0.2 / 1000,
                )
                time_value_spline_mcp = find_CFD_time_with_threshold(
                    time_array_event_mcp,
                    ampl_array_event_mcp,
                    a_spline_mcp[j],
                    0.2 / 1000,
                )
                time_value_landau_mcp = find_CFD_time_with_threshold(
                    time_array_event_mcp,
                    ampl_array_event_mcp,
                    a_landau_mcp[j],
                    0.2 / 1000,
                )
                time_value_skewG_mcp = find_CFD_time_with_threshold(
                    time_array_event_mcp,
                    ampl_array_event_mcp,
                    a_skewG_mcp[j],
                    0.2 / 1000,
                ...
                time_value_control_mcp = find_CFD_time_with_threshold_spline(
                    time_array_event_mcp,
                    ampl_array_event_mcp,
                    ampl_array_event_mcp.max(),
                    0.2,
                )
                time_value_para_mcp = find_CFD_time_with_threshold_spline(
                    time_array_event_mcp,
                    ampl_array_event_mcp,
                    a_para_mcp[j],
                    0.2 / 1000,
                )
                time_value_gaus_mcp = find_CFD_time_with_threshold_spline(
                    time_array_event_mcp,
                    ampl_array_event_mcp,
                    a_gaus_mcp[j],
                    0.2 / 1000,
                )
                time_value_lorentz_mcp = find_CFD_time_with_threshold_spline(
                    time_array_event_mcp,
                    ampl_array_event_mcp,
                    a_lorentz_mcp[j],
                    0.2 / 1000,
                )
                time_value_voigt_mcp = find_CFD_time_with_threshold_spline(
                    time_array_event_mcp,
                    ampl_array_event_mcp,
                    a_voigt_mcp[j],
                    0.2 / 1000,
                )
                time_value_spline_mcp = find_CFD_time_with_threshold_spline(
                    time_array_event_mcp,
                    ampl_array_event_mcp,
                    a_spline_mcp[j],
                    0.2 / 1000,
                )
                time_value_landau_mcp = find_CFD_time_with_threshold_spline(
                    time_array_event_mcp,
                    ampl_array_event_mcp,
                    a_landau_mcp[j],
                    0.2 / 1000,
                )
                time_value_skewG_mcp = find_CFD_time_with_threshold_spline(
                    time_array_event_mcp,
                    ampl_array_event_mcp,
                    a_skewG_mcp[j],
                    0.2 / 1000,
        ...
        for fam_ind, fam_proper in enumerate(
            [amp_fam_1_ind, amp_fam_2_ind, amp_fam_3_ind]
        ):
            ...
            spline_tr_params = gaussian_fit_binned_data(
                cfd20_spline_fit, "Interpolated spline"
            )
            ...
            rms_diff_para = np.sqrt(
                np.mean((np.array(cfd20_data_fit) - np.array(cfd20_para_fit)) ** 2)
            ).round(5)
            axes[0, 0].hist(
                cfd20_data_fit,
                bins=60,
            ...
            rms_diff_gaus = np.sqrt(
                np.mean((np.array(cfd20_data_fit) - np.array(cfd20_gaus_fit)) ** 2)
            ).round(5)
            axes[0, 1].hist(
                cfd20_data_fit,
                bins=60,
            ...
            rms_diff_lorentz = np.sqrt(
                np.mean((np.array(cfd20_data_fit) - np.array(cfd20_lorentz_fit)) ** 2)
            ).round(5)
            axes[1, 0].hist(
                cfd20_data_fit,
                bins=60,
            ...
            axes[1, 0].hist(
                ...
                + "\n"
                + r"$\Delta_{RMS}$ = "
                + str(rms_diff_lorentz),
            )

            rms_diff_voigt = np.sqrt(
                np.mean((np.array(cfd20_data_fit) - np.array(cfd20_voigt_fit)) ** 2)
            ).round(5)
            axes[1, 1].hist(
                cfd20_data_fit,
                bins=60,
            ...
            rms_diff_spline = np.sqrt(
                np.mean((np.array(cfd20_data_fit) - np.array(cfd20_spline_fit)) ** 2)
            ).round(5)
            axes[0, 2].hist(
                cfd20_data_fit,
                bins=60,
            ...
            rms_diff_landau = np.sqrt(
                np.mean((np.array(cfd20_data_fit) - np.array(cfd20_landau_fit)) ** 2)
            ).round(5)
            axes[1, 2].hist(
                cfd20_data_fit,
                bins=60,
            ...
            for i in range(2):
                for j in range(3):
                    axes[i, j].plot(
                        ...
                        "k--",
                        linewidth=2,
                        label=r"$\sigma_{tr}$ = " + str(round(data_tr_val, 1)) + " ps",
                    )
            ...
            axes[0, 0].plot(
                ...
                "r",
                linewidth=2,
                label=r"$\sigma_{tr}^{para}$ = " + str(round(para_tr_val, 1)) + " ps",
            )
            axes[0, 1].plot(
                x_tr_fit,
                gaus_tr_fit,
                "g",
                linewidth=2,
                label=r"$\sigma_{tr}^{Gaus}$ = " + str(round(gaus_tr_val, 1)) + " ps",
            )
            axes[1, 0].plot(
                x_tr_fit,
                lorentz_tr_fit,
                "blue",
                linewidth=2,
                label=r"$\sigma_{tr}^{Lorentz}$ = "
                + str(round(lorentz_tr_val, 1))
                + " ps",
            )
            axes[1, 1].plot(
                x_tr_fit,
                ...
                "orange",
                linewidth=2,
                label=r"$\sigma_{tr}^{Voigt}$ = " + str(round(voigt_tr_val, 1)) + " ps",
            )
            axes[0, 2].plot(
                x_tr_fit,
                spline_tr_fit,
                "purple",
                linewidth=2,
                label=r"$\sigma_{tr}^{spline}$ = "
                + str(round(spline_tr_val, 1))
                + " ps",
            )
            axes[1, 2].plot(
                x_tr_fit,
                ...
                "brown",
                linewidth=2,
                label=r"$\sigma_{tr}^{Landau}$ = "
                + str(round(landau_tr_val, 1))
                + " ps",
            )
            ...
            fig.suptitle(
                f"UFSD 3.2 W7 300V : Total {len(fam_proper)} signal events of Family {fam_ind+1}",
                fontsize=25,
                fontweight="bold",
            )
            plt.tight_layout(rect=[0, 0, 1, 0.96])
    ...
    if make_timewalk_plots:
        for fam_ind, fam_proper in enumerate(
            ...
            deltaT_para_fam = np.where(
                deltaT_para_fam > 0.05, deltaT_para_fam - 0.05, deltaT_para_fam
            )
            deltaT_gaus_fam = np.where(
                deltaT_gaus_fam > 0.05, deltaT_gaus_fam - 0.05, deltaT_gaus_fam
            )
            deltaT_lorentz_fam = np.where(
                deltaT_lorentz_fam > 0.05, deltaT_lorentz_fam - 0.05, deltaT_lorentz_fam
            )
            deltaT_voigt_fam = np.where(
                deltaT_voigt_fam > 0.05, deltaT_voigt_fam - 0.05, deltaT_voigt_fam
            )
            deltaT_spline_fam = np.where(
                deltaT_spline_fam > 0.05, deltaT_spline_fam - 0.05, deltaT_spline_fam
            )
            deltaT_landau_fam = np.where(
                deltaT_landau_fam > 0.05, deltaT_landau_fam - 0.05, deltaT_landau_fam
            )
            ...
            counts_para, bins_para, _ = axes[0, 0].hist(
                deltaT_para_fam,
                bins=num_bins_timewalk,
                range=(0.0, 0.05),
                color="r",
                edgecolor="black",
                label=r"$\Delta$t$_{para}$",
            )
            counts_gaus, bins_gaus, _ = axes[0, 1].hist(
                deltaT_gaus_fam,
                bins=num_bins_timewalk,
                range=(0.0, 0.05),
                color="g",
                edgecolor="black",
                label=r"$\Delta$t$_{Gaus}$",
            )
            counts_lorentz, bins_lorentz, _ = axes[1, 0].hist(
                deltaT_lorentz_fam,
                bins=num_bins_timewalk,
                range=(0.0, 0.05),
                color="blue",
                edgecolor="black",
                label=r"$\Delta$t$_{Lorentz}$",
            )
            counts_voigt, bins_voigt, _ = axes[1, 1].hist(
                deltaT_voigt_fam,
                bins=num_bins_timewalk,
                range=(0.0, 0.05),
                color="orange",
                edgecolor="black",
                label=r"$\Delta$t$_{Voigt}$",
            )
            counts_spline, bins_spline, _ = axes[0, 2].hist(
                deltaT_spline_fam,
                bins=num_bins_timewalk,
                range=(0.0, 0.05),
                color="purple",
                edgecolor="black",
                label=r"$\Delta$t$_{spline}$",
            )
            counts_landau, bins_landau, _ = axes[1, 2].hist(
                deltaT_landau_fam,
                bins=num_bins_timewalk,
                range=(0.0, 0.05),
                color="brown",
                edgecolor="black",
                label=r"$\Delta$t$_{Landau}$",
            )
            ...
            for i in range(2):
                for j in range(3):
                    ...
                    if time_res_calc:
                        ...
                        axes_RHS[i, j].set_ylabel(
                            r"$\sigma_{tr}$ of events in given bin / ps", fontsize=14
                        )
            ...
            if time_res_calc:
                ...
                bin_indices_para = (
                    np.digitize(deltaT_para_fam, bins_para, right=False) - 1
                )
                bin_indices_gaus = (
                    np.digitize(deltaT_gaus_fam, bins_gaus, right=False) - 1
                )
                bin_indices_lorentz = (
                    np.digitize(deltaT_lorentz_fam, bins_lorentz, right=False) - 1
                )
                bin_indices_voigt = (
                    np.digitize(deltaT_voigt_fam, bins_voigt, right=False) - 1
                )
                bin_indices_spline = (
                    np.digitize(deltaT_spline_fam, bins_spline, right=False) - 1
                )
                bin_indices_landau = (
                    np.digitize(deltaT_landau_fam, bins_landau, right=False) - 1
                )

                bin_indices_para[bin_indices_para == num_bins_timewalk] = (
                    num_bins_timewalk - 1
                )
                bin_indices_gaus[bin_indices_gaus == num_bins_timewalk] = (
                    num_bins_timewalk - 1
                )
                bin_indices_lorentz[bin_indices_lorentz == num_bins_timewalk] = (
                    num_bins_timewalk - 1
                )
                bin_indices_voigt[bin_indices_voigt == num_bins_timewalk] = (
                    num_bins_timewalk - 1
                )
                bin_indices_spline[bin_indices_spline == num_bins_timewalk] = (
                    num_bins_timewalk - 1
                )
                bin_indices_landau[bin_indices_landau == num_bins_timewalk] = (
                    num_bins_timewalk - 1
                )

                binned_para = [
                    np.where(bin_indices_para == i)[0] for i in range(num_bins_timewalk)
                ]
                binned_gaus = [
                    np.where(bin_indices_gaus == i)[0] for i in range(num_bins_timewalk)
                ]
                binned_lorentz = [
                    np.where(bin_indices_lorentz == i)[0]
                    for i in range(num_bins_timewalk)
                ]
                binned_voigt = [
                    np.where(bin_indices_voigt == i)[0]
                    for i in range(num_bins_timewalk)
                ]
                binned_spline = [
                    np.where(bin_indices_spline == i)[0]
                    for i in range(num_bins_timewalk)
                ]
                binned_landau = [
                    np.where(bin_indices_landau == i)[0]
                    for i in range(num_bins_timewalk)
                ]
                ...
                selected_tr_events_para = [
                    cfd20_para_fit[binned_para[i]] for i in range(num_bins_timewalk)
                ]
                selected_tr_events_gaus = [
                    cfd20_gaus_fit[binned_gaus[i]] for i in range(num_bins_timewalk)
                ]
                selected_tr_events_lorentz = [
                    cfd20_lorentz_fit[binned_lorentz[i]]
                    for i in range(num_bins_timewalk)
                ]
                selected_tr_events_voigt = [
                    cfd20_voigt_fit[binned_voigt[i]] for i in range(num_bins_timewalk)
                ]
                selected_tr_events_spline = [
                    cfd20_spline_fit[binned_spline[i]] for i in range(num_bins_timewalk)
                ]
                selected_tr_events_landau = [
                    cfd20_landau_fit[binned_landau[i]] for i in range(num_bins_timewalk)
                ]

                selected_para_tr_params = [
                    gaussian_fit_binned_data(selected_tr_events_para[i], "Parabolic")
                    for i in range(num_bins_timewalk)
                ]
                selected_gaus_tr_params = [
                    gaussian_fit_binned_data(selected_tr_events_gaus[i], "Gaussian")
                    for i in range(num_bins_timewalk)
                ]
                selected_lorentz_tr_params = [
                    gaussian_fit_binned_data(selected_tr_events_lorentz[i], "Lorentz")
                    for i in range(num_bins_timewalk)
                ]
                selected_voigt_tr_params = [
                    gaussian_fit_binned_data(selected_tr_events_voigt[i], "Voigt")
                    for i in range(num_bins_timewalk)
                ]
                selected_spline_tr_params = [
                    gaussian_fit_binned_data(
                        selected_tr_events_spline[i], "Interpolated spline"
                    )
                    for i in range(num_bins_timewalk)
                ]
                selected_landau_tr_params = [
                    gaussian_fit_binned_data(selected_tr_events_landau[i], "Landau")
                    for i in range(num_bins_timewalk)
                ]
                ...
                mcp_tr_est = 5
                per_bin_tr_val_para = np.sqrt(
                    np.where(
                        (1000 * np.array(selected_para_tr_params)[:, 2]) ** 2
                        - mcp_tr_est**2
                        < 0,
                        0,
                        (1000 * np.array(selected_para_tr_params)[:, 2]) ** 2
                ...
                per_bin_tr_val_gaus = np.sqrt(
                    np.where(
                        (1000 * np.array(selected_gaus_tr_params)[:, 2]) ** 2
                        - mcp_tr_est**2
                        < 0,
                        0,
                        (1000 * np.array(selected_gaus_tr_params)[:, 2]) ** 2
                ...
                per_bin_tr_val_lorentz = np.sqrt(
                    np.where(
                        (1000 * np.array(selected_lorentz_tr_params)[:, 2]) ** 2
                        - mcp_tr_est**2
                        < 0,
                        0,
                        (1000 * np.array(selected_lorentz_tr_params)[:, 2]) ** 2
                ...
                per_bin_tr_val_voigt = np.sqrt(
                    np.where(
                        (1000 * np.array(selected_voigt_tr_params)[:, 2]) ** 2
                        - mcp_tr_est**2
                        < 0,
                        0,
                        (1000 * np.array(selected_voigt_tr_params)[:, 2]) ** 2
                ...
                per_bin_tr_val_spline = np.sqrt(
                    np.where(
                        (1000 * np.array(selected_spline_tr_params)[:, 2]) ** 2
                        - mcp_tr_est**2
                        < 0,
                        0,
                        (1000 * np.array(selected_spline_tr_params)[:, 2]) ** 2
                ...
                per_bin_tr_val_landau = np.sqrt(
                    np.where(
                        (1000 * np.array(selected_landau_tr_params)[:, 2]) ** 2
                        - mcp_tr_est**2
                        < 0,
                        0,
                        (1000 * np.array(selected_landau_tr_params)[:, 2]) ** 2
                ...
                axes_RHS[1, 0].plot(
                    bin_centres_lorentz,
                    per_bin_tr_val_lorentz,
                    "o-",
                    color="k",
                    label="$\sigma_{tr}^{Lorentz}$",
                    markersize=10,
                    linewidth=2,
                    alpha=0.5,
                )
                axes_RHS[1, 1].plot(
                    bin_centres_voigt,
                    per_bin_tr_val_voigt,
                    "o-",
                    color="k",
                    label="$\sigma_{tr}^{Voigt}$",
                    markersize=10,
                    linewidth=2,
                    alpha=0.5,
                )
                axes_RHS[0, 2].plot(
                    bin_centres_spline,
                    per_bin_tr_val_spline,
                    "o-",
                    color="k",
                    label="$\sigma_{tr}^{spline}$",
                    markersize=10,
                    linewidth=2,
                    alpha=0.5,
                )
                axes_RHS[1, 2].plot(
                    bin_centres_landau,
                    per_bin_tr_val_landau,
                    "o-",
                    color="k",
                    label="$\sigma_{tr}^{Landau}$",
                    markersize=10,
                    linewidth=2,
                    alpha=0.5,
                )
            ...
            plt.tight_layout(rect=[0, 0, 1, 0.96])
            plt.savefig(
                f"./timewalk_UFSD_040425_family_{fam_ind+1}.png", dpi=300, facecolor="w"
            )
            plt.clf()
        ...
        if make_timewalk_plots & time_res_calc:
            for fam_ind, fam_proper in enumerate(
                ...
                skewG_tr_params = gaussian_fit_binned_data(
                    cfd20_skewG_fit, "Skewed Gaus"
                )
                skewG_tr_fit = gaussian(x_tr_fit, *skewG_tr_params)
                skewG_tr_val = np.sqrt((1000 * skewG_tr_params[2]) ** 2 - mcp_tr_est**2)

                rms_diff_skewG = np.sqrt(
                    np.mean((np.array(cfd20_data_fit) - np.array(cfd20_skewG_fit)) ** 2)
                ).round(5)
                axes[0].hist(
                    cfd20_data_fit,
                    bins=60,
                ...
                axes[0].plot(
                    ...
                    "yellow",
                    linewidth=2,
                    label=r"$\sigma_{tr}^{skewed~Gaus}$ = "
                    + str(round(skewG_tr_val, 1))
                    + " ps",
                )
                ...
                deltaT_skewG_fam = np.where(
                    deltaT_skewG_fam > 0.05, deltaT_skewG_fam - 0.05, deltaT_skewG_fam
                )

                counts_skewG, bins_skewG, _ = axes[1].hist(
                    deltaT_skewG_fam,
                    bins=num_bins_timewalk,
                    range=(0.0, 0.05),
                    color="yellow",
                    edgecolor="black",
                    label=r"$\Delta$t$_{skewed~Gaus}$",
                )
                ...
                axes_RHS[1].set_ylabel(
                    r"$\sigma_{tr}$ of events in given bin / ps", fontsize=14
                )
                axes_RHS[1].set_ylim(0, 65)

                bin_indices_skewG = (
                    np.digitize(deltaT_skewG_fam, bins_skewG, right=False) - 1
                )
                bin_indices_skewG[bin_indices_skewG == num_bins_timewalk] = (
                    num_bins_timewalk - 1
                )
                binned_skewG = [
                    np.where(bin_indices_skewG == i)[0]
                    for i in range(num_bins_timewalk)
                ]
                ...
                selected_tr_events_skewG = [
                    cfd20_skewG_fit_asnpa[binned_skewG[i]]
                    for i in range(num_bins_timewalk)
                ]
                selected_skewG_tr_params = [
                    gaussian_fit_binned_data(selected_tr_events_skewG[i], "Skewed Gaus")
                    for i in range(num_bins_timewalk)
                ]
                bin_centres_skewG = (bins_skewG[:-1] + bins_skewG[1:]) / 2
                per_bin_tr_val_skewG = np.sqrt(
                    np.where(
                        (1000 * np.array(selected_skewG_tr_params)[:, 2]) ** 2
                        - mcp_tr_est**2
                        < 0,
                        0,
                        (1000 * np.array(selected_skewG_tr_params)[:, 2]) ** 2
                ...
                axes_RHS[1].plot(
                    bin_centres_skewG,
                    per_bin_tr_val_skewG,
                    "o-",
                    color="k",
                    label="$\sigma_{tr}^{skewed~Gaus}$",
                    markersize=10,
                    linewidth=2,
                    alpha=0.5,
                )
