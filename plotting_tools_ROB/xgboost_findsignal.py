...
def read_root_files(file_list):
    ...
    for file in file_list:
        ...
        df = pd.DataFrame(
            {"pmax": pmax, "rms": rms, "charge": charge, "negpmax": negpmax, "cfd": cfd}
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Signal and background classifier using specific branches from ROOT files"
    )
    parser.add_argument(
        "--train_files",
        nargs="+",
        help="List of ROOT files for training",
        required=True,
    )
    parser.add_argument(
        "--eval_file", help="ROOT file to evaluate the model", required=True
    )
